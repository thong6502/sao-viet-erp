...
- **Orchestrator (2026-07-18) — UI Chunk 7 (Kế hoạch ACTIONS: ghép bài + duyệt/gán máy/phát): XONG + BROWSER-VERIFY PASS.** File mới `GhepBaiView.tsx` (composer 2 cột) · `LsxToast.tsx`; additive `client.ts` (api ghi duyetMau/ghep/ganMay/phat) · `SanXuatPage`/`List`/`Detail`/`css`. `tsc` PASS. **Chạy FE:5173 → Kế hoạch SX → Ghép bài:** composer render đẹp, **tiêu đề+ghi chú nêu rõ MÁY CHỈ GHI NHẬN** (không lọc/chặn/so số con); click lệnh → vào khay + **auto-prefill giấy/màu từ PTG** (verify TƯƠNG TÁC OK); phát **per-TỜ** (đúng — tờ ghép đa-lệnh), blocker cổng AND client-side + 422 backend. → còn **UI Chunk 8 (màn thợ TEXTLESS)** + Theo dõi/Kanban (placeholder).
