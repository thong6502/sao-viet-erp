...
| SEAM-04 | Kinh doanh · quotation_ref/deposit_payment (port) — spec-10 Đơn hàng bán · `backend/app/services/order_ports.py` | Báo giá + Tài chính (Payment) | port-interface | `quotation_ref(quotation_id) -> {approved, version, effective_from, total, customer_id, lines}` + `record_deposit/deposit_total(order_id) -> Payment(kind=deposit)`; gate ③→④ `approved AND deposit≥total·min_deposit_pct` | `test_seam_04_deposit_backfilled` (XANH, live) + `test_seam_04_deposit_needs_repo` (XANH) + `test_seam_04_quotation_ref_needs_repo` (XANH) — `backend/tests/test_seam_don_hang_ban.py` | KD→Báo giá/TC (downstream KD sở hữu port) | ✅ | ĐÃ ĐÓNG (feat-048, Pha A1): cả 2 nửa live — quotation_ref (`QuotationRefAdapter`, Báo giá feat-043..045) + deposit_payment (`DepositPaymentAdapter` trên bảng `payments`); module-level vẫn raise cho mis-wire; cọc → mở cổng chốt ③→④ |
...
| SEAM-13 | Kinh doanh · bao_gia · CostingResultPort (port) — spec-09 Báo giá · `backend/app/services/quotation_ports.py` | Kinh doanh · Tính giá (spec-08, costing engine) | port-interface | `get_costing_result(costing_id, quantity?, cost_hint?) -> {cost_von_total, unit_price_snapshot, norm_snapshot, effective_from/to, quantity, quantity_options}` (Báo giá snapshot copy-on-write giá **và** định mức theo mức bậc-SL; §34 L877-878, §43#5 L1209) | `test_seam_13_costing_result` (XANH — `EstimateCostingAdapter`) + `test_costing_picker_pulls_calculated_estimate` (e2e) — `backend/tests/test_seam_quotation.py`, `test_quotations_api.py` | Báo giá→Tính giá (downstream Báo giá sở hữu port) | ✅ | ĐÃ ĐÓNG (2026-07-03): engine Tính giá (Phase 2A) trả EstimateOption đông cứng → `EstimateCostingAdapter` đọc estimate `calculated` (repo inject ở `deps.get_quotation_service`); gọi thiếu repo vẫn raise (fail loudly), test XANH |
...
| SEAM-16 | Kinh doanh · Khách hàng (CRM) · `CustomerReceivablePort` (port) — spec-06 · `backend/app/ports/customer_finance_port.py` | Tài chính–Kế toán · công nợ (`cong_no`) | port-interface | `get_ar_balance(customer_id) -> int (VND)` — dư nợ AR cho thẻ công nợ chỉ-đọc; §23 L528, §41 L1133 | `test_seam_16_stub_raises` — `backend/tests/test_seam_customer_finance.py` · `test_man_khach_hang_va_man_cong_no_khong_duoc_lech` — `backend/tests/test_sales_invoices_api.py` | CRM→Kế toán (downstream CRM sở hữu port) | ✅ | ĐÃ ĐÓNG (12/08/2026): `deps.AccountingReceivablePort` gọi THẲNG `accounting_service.receivable_rows` — CÙNG hàm màn Công nợ phải thu dùng, không còn bản chép. Dư nợ nay chỉ đếm HÓA ĐƠN đã ghi nhận (`sales_invoices`), không còn đếm đơn đã chốt. |
...
| SEAM-25 | Thu mua · PMH (`purchase_requests`) | Kế toán · Phiếu chi/UNC (`payment_vouchers`) | direct FK + service contract | `payment_vouchers.purchase_request_id → purchase_requests.id`; Kế toán duyệt PMH và có thể lập PC/UNC trong cùng giao dịch; truy YCMH qua `purchase_request_sources`; đối soát bằng `amount_vnd` | `backend/tests/test_accounting_api.py` | Thu mua→Kế toán | ✅ | ĐÃ ĐÓNG (2026-07-10): hộp thư PMH, duyệt/từ chối, PC/UNC nhiều đợt, tài khoản ngân hàng, phân quyền và truy vết đều chạy thật |
