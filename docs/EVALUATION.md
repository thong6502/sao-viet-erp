...
<!-- The Score Log holds one example row until a real spec is evaluated. -->
...
| Date | Spec | design | originality | craft | functionality | overall | verdict | notes |
|------|--------|:------:|:-----------:|:-----:|:-------------:|:-------:|:-------:|-------|
...
| 2026-06-26 | feat-011 | 4 | 4 | 5 | 4 | 4 | PASS | Activity Log (read-only): GET /api/audit + ActivityLogPage table. As admin the log surfaced the full real history written across feat-004..010 — create/rename/delete role, role-permission edit, create/delete department, create user, assign role, lock + unlock user — each with actor "Admin", a timestamp, a friendly Vietnamese action label, target, and detail (e.g., "abc → Cộng tác viên", "tranb@example.com → dept:3"). The filter-by-action dropdown narrowed the table correctly; console clean. Empty/loading/error/403 states are coded (empty-state not browser-triggered since the live log has data) → functionality 4. |
| 2026-06-26 | feat-010 | 4 | 4 | 5 | 4 | 4 | PASS | Menu/route gating: the sidebar now shows only modules the role can Read (GET /api/auth/permissions). As admin: Tổng quan + Kinh doanh (6) + Quản trị; Sản xuất/Kho/Thu mua hidden (their modules aren't in the catalog). Logged in as Trần B (NV Sales): only Dashboard + Báo giá/Đơn hàng bán/Khách hàng — Sản phẩm/Tính giá thành/Hợp đồng and the whole Quản trị section are hidden. Empty sections are dropped; console clean. The forbidden-route 403 banner is code-covered (hidden items aren't navigable from the gated sidebar) → functionality 4. |
| 2026-06-26 | feat-009 | 4 | 4 | 5 | 4 | 4 | PASS | Người dùng admin screen: table + detail. Created "Trần B" in Kinh doanh (201) → row shows "Chưa gán" + "Hoạt động"; empty-name → inline "Họ tên là bắt buộc"; the role dropdown listed ONLY the user's department roles (KD: Trưởng phòng KD/NV Sales/QA Reviewer — not Giám đốc) and saving assigned NV Sales; locking → "Đã khóa"; the admin's own lock button is disabled with "Không thể tự khóa…". Console clean; network create 201 / role 200 / active 200. Lock→login/me rejection is backend-tested (not browser-driven to avoid replacing the admin session); "minimal default role" implemented as no-role-yet (most minimal) → functionality 4. |
| 2026-06-26 | feat-008 | 4 | 4 | 5 | 4 | 4 | PASS | Phòng ban admin screen: department list with summaries (role_count · user_count · head — Ban giám đốc 1/1/Admin, HCNS 2/0, Kinh doanh 3/0) + master-detail (rename, head picker, roles-in-dept, delete). Created "Thiết kế" (201, appears, empty-state: head picker disabled + "Chưa có vai trò"); duplicate "Kinh doanh" → inline "Tên phòng ban đã tồn tại" (409); deleting in-use Kinh doanh blocked (409 "phòng còn 3 vai trò"); deleting the empty "Thiết kế" (204) removed it. Console clean apart from the deliberate 409s. Rename + set-head verified by backend tests (same updateDepartment path), not separately browser-driven → functionality 4. |
| 2026-06-26 | feat-007 | 4 | 4 | 5 | 5 | 4 | PASS | feat-007 follow-up — role RENAME + DELETE. Renamed "abc" → "Cộng tác viên" (PUT 200, header + dropdown updated); deleted the unused role (DELETE 204, selection moved to the next role); deleting the in-use "Giám đốc" was blocked (DELETE 409) with inline "Không thể xóa: còn 1 người dùng…". Backend test_rbac_roles_api 8 tests (rename, dedup-on-rename, delete-not-in-use, delete-in-use-blocked); init.ps1 33 passed. Console clean apart from the deliberately-triggered 409. |

| 2026-06-27 | feat-016 | 4 | 4 | 5 | 4 | 4 | PASS | Frontend silent-refresh + cookie session restore + server-side logout (spec-03). Golden journey on a fresh browser, live backend on :8000 (new schema) + Vite :5173: Login renders (anonymous; startup POST /api/auth/refresh → 401, app shows Login, not a blank screen). Login admin@example.com → Dashboard ("Signed in admin@example.com", "Welcome, Admin.") inside the ERP AppShell+Sidebar; login 200. localStorage is EMPTY — no bearer token persisted (access token in memory only). Full reload → restored to Dashboard via POST /api/auth/refresh 200 (refresh httpOnly cookie; rotated). Log out → returns to Login, POST /api/auth/logout 204 (idempotent). Reload after logout → stays on Login, /refresh 401 (cookie revoked+cleared). Console clean apart from benign favicon 404 + the deliberately-triggered 401s the app handles (the doubled /refresh and /logout are dev-only React StrictMode effect double-invoke; single in production). Silent-refresh-on-access-expiry + "at most one /refresh on concurrent 401s" (shared in-flight promise) are covered by backend tests (test_token_version expired→401, test_refresh_api rotation/reuse) + the authed() refresh-and-retry code, not browser-exercised (15-min access token) → functionality 4. UI unchanged from feat-003 (built to UI_DESIGN tokens). Threshold: every criterion ≥ 4. |
