...
> 🔴 `salary_mechanism` · `probation_ratio` (bộ nguyên tắc lương theo phòng, DORMANT từ 07/09/2026)
> **ĐÃ DROP** 17/09/2026 (mg `0307`). Tỷ lệ thử việc là tham số công ty `payroll_params.probation_ratio`
> (không đụng). Mất giá trị đã khai — không khôi phục được.
...
| `has_piece_work` | `Boolean` → `BOOLEAN` | — | no | `false` | Phòng sản xuất có lương khoán theo sản lượng (nối engine khoán ở pha sau). Cờ lương DUY NHẤT còn lại ở cấp phòng. |
...
| `anh_minh_hoa` | `String(500)` → `VARCHAR(500)` | — | yes | — | **Ảnh minh họa in cho khách** (migration 0295) — URL đọc qua `/api/files/bao-gia/...`, hiện ở cột "Hình ảnh minh họa" của bản in (cột thay chỗ "Thành tiền" đã bỏ). Thuộc CỤM IN chứ không thuộc dòng: mọi dòng cùng nhãn `nhom` / cùng `product_name` mang CHUNG một URL, đặt/xóa ở dòng nào cũng ghi cả cụm. NULL = chưa có ảnh, bản in để trống ô. |
...
| `family_id`  | `String(36)` → `VARCHAR(36)`                           | **IX**                  | no   | —              | Rotation-chain id. Reusing a revoked token revokes every sibling in the family (theft signal), except within 60 s of its rotation while the family is alive. |
...
> 🔴 Bậc tay nghề **ĐÃ DROP** 17/09/2026 (mg `0305`): cột `job_grade_id`, `job_grade` (chữ cũ),
> `pay_grade_key` và bảng `job_grades`. Hồ sơ không còn ô bậc; "Nâng bậc" nay là **Đổi chức danh**
> (`promote` chỉ đổi `position`). Mất bậc đã khai — không khôi phục được.
> Cột `payroll_group` (nhóm lương — trục tra `salary_rate_rules`) **ĐÃ DROP** cùng index
> `ix_employees_payroll_group` 17/09/2026 (mg `0307`); cột "Nhóm lương" của file Excel hồ sơ cũng bỏ.
...
### ~~`job_grades`~~ — ĐÃ DROP 17/09/2026 (mg `0305`)

Danh mục bậc tay nghề (Bậc 1…5 + hệ số chia sản lượng `output_coefficient`) đã gỡ hẳn cùng mọi cột trỏ
vào nó (`employees.job_grade_id`, `san_xuat_khoang_tham_gia.job_grade_id/output_coefficient`,
`san_xuat_phan_bo_dong.he_so_bac`). Chia sản lượng khoán nay theo phút có mặt hợp lệ, không nhân hệ số.
...
dòng mỗi khi đổi giai đoạn (status / department / position) — timeline theo `effective_date`
...
| `id`             | `Integer` → `INTEGER` / `SERIAL`                       | **PK**                      | no   | auto-increment | Surrogate primary key.                                                                    |
| `employee_id`    | `Integer` → `INTEGER`                                  | **FK→employees.id**, **IX** | no   | —              | Nhân viên chủ; `ON DELETE CASCADE`.                                                       |
| `event_type`     | `String(24)` → `VARCHAR(24)`                           | —                           | no   | —              | hired/confirmed/transferred/promoted/leave_start/leave_end/suspended/unsuspended/resigned/reinstated. |
| `effective_date` | `Date` → `DATE`                                        | **IX**                      | yes  | —              | Ngày hiệu lực của giai đoạn.                                                              |
| `field`          | `String(40)` → `VARCHAR(40)`                           | —                           | yes  | —              | Trường thay đổi (vd "status", "department", "position"; mốc cũ có thể còn "job_grade"). |
...
| `suggested_next_day` | `Boolean` → `BOOLEAN`                                  | —                       | no   | `false`   | Giờ gợi ý rơi **sang hôm sau** (ca đêm quên bấm RA 06:00 sáng). Không có cờ này thì punch bù dính ngày công ⇒ 06:00 bị gom về hôm trước, ngày treo vẫn treo. Migration 0267 (07/09/2026). |
...
| `late_off_days_json`, `ca_lam_json` | `Text` → `TEXT` | — | yes | — | `late_off_days_json`: JSON list SỐ PHÚT vi phạm (trễ+sớm, không phép) MỖI NGÀY — đóng băng để Lương áp bảng phạt trễ/sớm tự động (mỗi phần tử = 1 lần). Thêm qua migration 0098. `ca_lam_json`: đời cũ `{ca: [công/ngày]}`; **từ 08/09/2026** `{"ca": {...}, "muc": {ca: {"meal", "shift"}}}` — mức cơm ca / phụ cấp ca ĐÓNG BĂNG lúc chốt công (bản rà liên thông B6), `_load_ca_lam`/`_load_ca_muc` đọc được cả hai đời, không cần migration. |
...
- One department request has many `yeu_cau_mua_nguon_lenh` (lệnh/bài được mua cho, mg 0325).
...
### `yeu_cau_mua_nguon_lenh`

**Purpose:** YCMH mua MẶT HÀNG nào cho LỆNH/BÀI GHÉP nào (mg 0325, 18/09/2026). Ghi lúc bấm "Đề nghị mua" ở Kế hoạch vật tư — mỗi dòng thiếu được tick là một hàng. "Ngày cần" của lệnh trên Kế hoạch vật tư đọc ngược qua bảng này = `needed_date` SỚM NHẤT của các yêu cầu còn hiệu lực; không có liên kết (tồn đủ, không mua) thì để trống. Hiệu lực = yêu cầu chưa `cancelled` VÀ còn dòng sống (`cancelled_at IS NULL`) cùng `(hang_loai, hang_id)`. Sửa yêu cầu không đụng tới bảng này.

| Column                  | Type (SQLAlchemy → SQLite / Postgres) | Key                                            | Null | Default        | Meaning |
| ----------------------- | ------------------------------------- | ---------------------------------------------- | ---- | -------------- | ------- |
| `id`                    | `Integer` → `INTEGER` / `SERIAL`      | **PK**                                         | no   | auto-increment | Surrogate primary key. |
| `department_request_id` | `Integer` → `INTEGER`                 | **FK→department_purchase_requests.id**, **IX** | no   | —              | Yêu cầu mua (cascade). |
| `hang_loai`             | `String(8)` → `VARCHAR(8)`            | —                                              | no   | —              | Mặt hàng gốc: `giay` \| `vat_tu`. |
| `hang_id`               | `Integer` → `INTEGER`                 | —                                              | no   | —              | Id trong `giay_nguyen` / `vat_tu_in_an`. Soft ref. |
...
- Primary key: `id`. Indexes on `department_request_id`, `lsx_id`, `bai_ghep_id`.
- Check `ck_ycmh_nguon_lenh_mot_chu_the`: `(lsx_id IS NULL) <> (bai_ghep_id IS NULL)`.

**Relationships**

- Many links belong to one `department_purchase_requests` row (`DepartmentPurchaseRequest.nguon_lenh`).
...
| `meal_allowance`  | `Numeric(14,2)` → `NUMERIC(14,2)`                    | —      | no   | `25000`        | Phụ cấp cơm khai theo CA. Engine Lương cộng trọn mức cho mỗi ngày làm ca đạt ngưỡng `payroll_params.phu_cap_ca_min_cong` (khối `ca_lam` của `_compute`). Thêm qua migration 0094. |
| `shift_allowance` | `Numeric(14,2)` → `NUMERIC(14,2)`                    | —      | no   | `50000`        | Phụ cấp CA (áp ca ngày hay đêm) khai theo CA. Engine Lương cộng trọn mức cho mỗi ngày làm ca đạt ngưỡng — chỗ khai "tiền cục theo ca" (ca đêm 77k/đêm, ca tới sáng 125k…, chủ 07/09/2026). Đổi tên từ `night_allowance` qua migration 0095. |
...
| `break_start_minute` | `Integer` → `INTEGER`                             | —      | yes  | —              | **Nghỉ giữa ca — bắt đầu** (phút từ 0h). NULL cả hai = ca không khai nghỉ ⇒ engine chạy như cũ. Khai vào thì khung tính công = (ra − vào) − nghỉ; phút rơi vào khoảng nghỉ không là làm, không là trễ/sớm (ca 7:30–16:30 nghỉ 11:30–12:30 ⇒ mẫu số 480', nửa buổi = 0,5 công — khớp bảng lương tay). Ca qua đêm: mốc nhỏ hơn giờ vào ca hiểu là rơi sang hôm sau. Thêm qua migration 0267 (07/09/2026). |
| `break_end_minute`   | `Integer` → `INTEGER`                             | —      | yes  | —              | Nghỉ giữa ca — kết thúc (phút từ 0h). Cùng luật với cột trên. Migration 0267. |
...
| `bhxh_rate_er` | `Numeric(6,4)` | no | `0.175` | Tỷ lệ NSDLĐ đóng BHXH — **KHÔNG trừ vào lương NV**, chỉ tính chi phí công ty. Thêm qua migration 0076. **DORMANT 07/09/2026** — chủ bỏ cột NSDLĐ khỏi màn (không dùng tới): `ParamsIn`/`update_params` không nhận, engine chưa bao giờ đọc; chỉ còn `tnld_bnn_rate` cho ca "BH đóng ở nơi khác". Giữ cột, không drop. |
| `bhyt_rate_er` | `Numeric(6,4)` | no | `0.03` | Tỷ lệ NSDLĐ đóng BHYT (không trừ vào lương NV). Thêm qua migration 0076. **DORMANT 07/09/2026** — chủ bỏ cột NSDLĐ khỏi màn (không dùng tới): `ParamsIn`/`update_params` không nhận, engine chưa bao giờ đọc; chỉ còn `tnld_bnn_rate` cho ca "BH đóng ở nơi khác". Giữ cột, không drop. |
| `bhtn_rate_er` | `Numeric(6,4)` | no | `0.01` | Tỷ lệ NSDLĐ đóng BHTN (không trừ vào lương NV). Thêm qua migration 0076. **DORMANT 07/09/2026** — chủ bỏ cột NSDLĐ khỏi màn (không dùng tới): `ParamsIn`/`update_params` không nhận, engine chưa bao giờ đọc; chỉ còn `tnld_bnn_rate` cho ca "BH đóng ở nơi khác". Giữ cột, không drop. |
...
| `ca_khop_gio_chuan` | `Boolean` | no | `true` | **Ca phải khớp giờ công chuẩn** (07/09/2026): khai ca mà (giờ ra − giờ vào − nghỉ giữa ca) ≠ `standard_hours_per_day` thì không cho lưu — hai mẫu số công/tiền giờ phải cùng một số. Tắt chỉ khi có ca bán thời gian. Migration 0268. |
...
> từ 07/09/2026 (engine không tra, route `/api/luong/rules` đã gỡ), nay drop hẳn cùng cơ chế lương cũ.
> Mức lương chỉ lấy từ `employee_salaries` của từng người. Mất các quy tắc đã khai — không khôi phục được.
...
> 🔴 `amount_mode` (rule/manual/dept_row) · `source_salary_row_id` **ĐÃ DROP** 17/09/2026 (mg `0307`):
> mọi mốc lương là số khai riêng của người đó; `POST /api/luong/salaries/{id}` không nhận `amount_mode` nữa.
...
| `base_amount`    | `Numeric(14,2)` | —                           | yes  | —       | DORMANT: số tổng khai tay của bản ghi cũ — engine chỉ dùng làm fallback khi mốc chưa tách 2 ô vị trí/trách nhiệm. |
...
| `status`        | `String(12)`    | **IX**                      | no   | `pending` | pending/approved/**paid**/rejected/cancelled. `paid` = kế toán đã lập phiếu chi (mg 0270, 07/09/2026) — CHỈ trạng thái này mới trừ vào lương; huỷ phiếu chi ⇒ về `approved` (chặn nếu kỳ lương đã chốt). |
...
| `advance_total` | `Numeric(14,2)` | — | no | `0` | Tổng tạm ứng **ĐÃ CHI** (kind=tam_ung, `salary_advances.status = paid`) của kỳ — từ 07/09/2026 phiếu mới duyệt chưa lập phiếu chi KHÔNG trừ (mg 0270). |
| `luong_dot_1_total` | `Numeric(14,2)` | — | no | `0` | Tổng "thanh toán lương đợt 1" đã duyệt (kind=luong_dot_1) của kỳ. Migration 0108. |
| `no_ung_ky_truoc` | `Numeric(14,2)` → `NUMERIC(14,2)` | — | no | `0` | **Nợ tạm ứng kỳ TRƯỚC mang sang** (mg 0270, chủ chốt 07/09/2026: "trừ không hết thì chuyển sang tháng sau") — chép từ `no_ung_chuyen_ky_sau` của dòng kỳ trước lúc Tính lại. Tạm ứng + đợt 1 + nợ này trừ SAU CÙNG (sau BHXH · đoàn phí · thuế · khấu trừ khác). Chốt lương chặn (L14) khi kỳ trước tính lại làm số này lạc hậu. |
| `no_ung_chuyen_ky_sau` | `Numeric(14,2)` → `NUMERIC(14,2)` | — | no | `0` | **Phần tạm ứng kỳ này CHƯA TRỪ HẾT**, chuyển sang kỳ sau (mg 0270). Cả `generate` lẫn `update_line` ghi qua `_tru_tam_ung_sau_cung`. Chốt kỳ chỉ CẢNH BÁO (không chặn) người còn nợ / thực lĩnh 0. |
...
| `in_insurance_base` | `Boolean` | — | no | `false` | Có cộng vào gốc đóng BH không (mặc định KHÔNG — gốc đóng BH là `luong_vi_tri`). **DORMANT 07/09/2026** — engine không đọc (mức đóng BH = vị trí + trách nhiệm, chốt 12/08); API không nhận/trả nữa. Giữ cột. |
...
- Relationships: referenced by `employee_salary_components.component_id` (ON DELETE CASCADE).
...
| `source` | `String(8)` | — | no | `employee` | NGUỒN dòng này: `employee` = chép từ hồ sơ NV ⇒ bị GHI ĐÈ mỗi lần "Tính lại" (trừ dòng `da_de_tay`); `line` = HCNS thêm tay cho RIÊNG kỳ này (thưởng nóng) ⇒ GIỮ NGUYÊN qua mọi lần tính lại và KHÔNG lặp sang kỳ sau. `auto` = **LEGACY** (hoa hồng KD, mg 0227, 21/08–07/09/2026) — từ mg 0269 hoa hồng là cột `payroll_lines.hoa_hong`, không còn dòng `auto` nào được sinh, dữ liệu cũ đã chuyển vào cột và xoá. |
...
**Purpose:** danh mục **Công việc khoán** — đơn giá khoán theo tổ + đơn vị. Từ 17/08/2026 khai ở
Cấu hình danh mục (`loai = cong_viec_khoan`, quyền `dm_cong_viec_khoan`, nền `CatalogRepo`); trước
đó là một tab của màn Lương. Vẫn là bảng giá mà Lương khoán tra — chỉ đổi chỗ khai.

| Column | Type | Key | Null | Default | Meaning |
|---|---|---|---|---|---|
| `id` | `Integer` | **PK** | no | auto | PK. |
| `ma` | `String(20)` | — | yes | — | Mã. Máy cấp `KH-####`; mã cũ của xưởng (`BE-01`, A–F bảng giấy) giữ nguyên. **Đổi tên từ `code`, mg `0210`.** |
| `ten` | `String(255)` | — | no | — | Tên công việc khoán. **Đổi tên từ `name`, mg `0210`.** |
...
| `unit` | `String(24)` | — | no | `khác` | ĐVT — lưu **MÃ** danh mục `don_vi_do` (`to`, `kg`, `m2`), cùng lối `giay.don_vi_gia`. mg `0210` đổi dòng cũ từ TÊN sang MÃ khi khớp chính xác; chữ ngoài danh mục vẫn lưu được (dòng cũ · seed · import). |
| `unit_price` | `Numeric(14,2)` | — | no | — | Đơn giá/đơn vị. |
| `cong_thuc_khoan` | `Text` | — | yes | — | **CÁCH ĐO LƯỢNG KHOÁN** — tab "Công thức khoán" của màn Công việc khoán (18/09/2026, mg `0317`). Ra LƯỢNG theo `unit`; màn Khoán theo kỳ của kế toán nhân với `unit_price`. **Tầng sản xuất KHÔNG chạy công thức này** — bàn tổ chỉ ghi nhận số lượng. Quay ngược mg `0274`: lý do hồi đó (cùng đầu việc làm ở hai công đoạn thì đếm khác nhau) tan khi mỗi công việc khoán mang MÃ RIÊNG của xưởng đã mã hoá sẵn khổ / số màu / số lớp. Migration chép từ `cong_doan_dau_viec.cong_thuc_khoan`, chỉ điền ô đang trống. |
| `note` | `String(255)` | — | yes | — | Ghi chú. |
| `active` | `Boolean` | — | no | `true` | Còn dùng. Còn nơi dùng thì luật xoá chung chỉ tắt cờ này (xem `danh_muc_tham_chieu._cong_viec_khoan`). **Đổi tên từ `is_active`, mg `0210`.** |
| `created_at` | `DateTime(tz)` | — | no | now | Khi tạo. |

> 🔴 **`group_name` + `department_id` GỠ 17/09/2026 (mg `0311`)** — nhãn tổ trên dòng và MỘT tổ sở
> hữu. Một việc nay làm được ở NHIỀU tổ ⇒ danh sách tổ ở bảng nối `cong_viec_khoan_to`. Migration
> chép `department_id` (hoặc tổ trùng tên `group_name` khi dòng chưa gắn tổ) sang bảng nối rồi mới
> gỡ cột.

> 🔴 Bảng `piece_leader_bonus_brackets` (bậc thưởng/phạt tổ trưởng theo khoảng sản lượng × tỷ lệ
> lỗi KCS) và cột `payroll_lines.thuong_to_truong` **ĐÃ DROP** 13/09/2026 (mg `0300`) — bỏ hẳn
> tính năng, không còn API/màn khai.

---

### `cong_viec_khoan_to`

**Purpose:** các **TỔ làm một công việc khoán** (17/09/2026, mg `0311`) — bảng nối `piece_rates` ↔
`departments`. Cùng một việc ("Bế nổi") do tổ Bế lẫn tổ Thành phẩm làm, chung một đơn giá. Khai ở ô
"Tổ làm việc này" (chọn nhiều) của drawer Công việc khoán; thân POST/PUT `/api/cong-viec-khoan` mang
`department_ids` (vắng = giữ nguyên, rỗng = lỗi). Mọi chỗ khớp "đầu việc của tổ X" đọc
`X in piece_rates.department_ids`: định mức đầu việc của công đoạn, đầu việc gợi ý của bước lệnh,
panel "Đơn giá khoán của tổ" bên Cấu hình lương, tab lọc theo tổ (việc nhiều tổ hiện ở mỗi tab).

| Column | Type | Key | Null | Default | Meaning |
|---|---|---|---|---|---|
| `piece_rate_id` | `Integer` | **PK**, **FK→piece_rates.id** | no | — | Công việc khoán; `ON DELETE CASCADE` + ORM `delete-orphan` (bỏ tổ khỏi danh sách là xoá dòng). |
| `department_id` | `Integer` | **PK**, **IX** | no | — | Tổ (ref `departments.id`, soft-ref không FK cứng). Tổ bị xoá khỏi cây tổ chức thì dòng ở lại, form đánh dấu để người khai tự gỡ. |

---

### `cong_viec_khoan_phat_sinh`

**Purpose:** **VIỆC PHÁT SINH** của một công việc khoán (14/09/2026) — vd "In 4 màu" có "Thay kẽm ·
100 đ/bản". Thứ bậc: tổ → công đoạn → công việc khoán → việc phát sinh, nên dòng chỉ khai BA ô (tên
việc · đơn giá · đơn vị tính); tổ và công đoạn đọc ở `piece_rates` cha. Khai trong drawer Công việc
khoán (thân POST/PUT `/api/cong-viec-khoan` mang `viec_phat_sinh`), dùng chung quyền · nhật ký · nhân
bản · xoá của cha. Đợt đầu CHỈ khai báo + hiển thị — sản xuất chưa đọc bảng này. Bảng MỚI ⇒
`create_all` tự dựng, không migration.

| Column | Type | Key | Null | Default | Meaning |
|---|---|---|---|---|---|
| `id` | `Integer` | **PK** | no | auto | PK. **Giữ nguyên qua các lần lưu** (repo sửa tại chỗ theo id, không xoá-rồi-chèn) — sau này sản xuất ghi số lượng trỏ vào id, đổi tên việc không làm mồ côi lần ghi cũ. |
| `piece_rate_id` | `Integer` | **FK→piece_rates.id**, **IX** | no | — | Công việc khoán cha; `ON DELETE CASCADE` + ORM `delete-orphan` (bỏ dòng khỏi danh sách là xoá hàng). |
| `ten` | `String(255)` | — | no | — | Tên việc. Không trùng trong cùng một công việc khoán (so bỏ hoa/thường + khoảng trắng thừa, service kiểm — không có UNIQUE ở DB để đổi tên chéo hai dòng trong một lần lưu không vấp thứ tự flush). |
| `don_gia` | `Numeric(14,2)` | — | no | — | Đơn giá cho MỘT đơn vị; ≥ 0. |
| `don_vi` | `String(24)` | — | no | — | MÃ đơn vị trong `don_vi_do` (`kem`, `luot`), lưu chữ thường. Khác `piece_rates.unit`: mã ngoài danh mục bị CHẶN (bảng mới, không có dòng cũ cần đỡ). |
| `thu_tu` | `Integer` | — | no | `0` | Thứ tự đã khai trên form (0, 1, 2…). |
| `created_at` | `DateTime(tz)` | — | no | now | Khi tạo. |
...
**Tất cả cột:** `id`, `ma`, `ten`, `loai_may`, `hang_san_xuat`, `model`, `so_seri`, `ghi_chu`, `toc_do`, `toc_do_min`, `toc_do_max`, `don_vi_toc_do`, `makeready_time_default`, `cho_ky_thuat_gio`, `kho_max_dai`, `kho_max_rong`, `kho_min_dai`, `kho_min_rong`, `kho_kem_dai`, `kho_kem_rong`, `vung_in_dai`, `vung_in_rong`, `nhip_giay_mm`, `le_hong_mm`, `duoi_thang_mau_mm`, `fields_theo_loai`, `active`, `created_at`, `updated_at`. **`active`** (BOOLEAN NOT NULL DEFAULT TRUE, migration `0202`) = máy CÒN DÙNG hay đã thanh lý — cờ của nút "Ngừng dùng" trên màn Máy, nhờ nó màn này vào được luật xoá chung của danh mục. ĐỪNG NHẦM với `trang_thai` đã gỡ ở mg `0186`: cái cũ trộn ba nghĩa (đang chạy / bảo trì / đã nghỉ) và không có ô nhập nào. Máy dừng **TẠM** (bảo trì, hỏng) vẫn `active=TRUE` và khai bằng khoảng thời gian ở `machine_unavailable_periods` — đó vẫn là thứ Xếp lịch đọc để né khe. `xep_lich_service._may_lam_duoc` lọc `active`, nhưng luôn GIỮ máy đang gán để lệnh cũ mở ra không trống ô. 🔴 **`so_nhan_cong` GỠ 06/09/2026 (mg `0270`)** — ô "Số người vận hành tiêu chuẩn" của máy. Cả hệ nay chỉ còn MỘT con số nhân lực là `cong_doan_dau_viec.so_nguoi_tieu_chuan`: kíp của mọi loại bước (máy · tổ · thuê ngoài) đều điền sẵn từ đó. Khai kíp ở máy là nguồn sự thật thứ hai cho cùng một câu hỏi, và không màn nào bày hai số cạnh nhau để ai đó phát hiện chúng lệch. Hệ quả chấp nhận: hai máy cùng một công đoạn nhận cùng một kíp. **Nhíp GIẤY:** `nhip_giay_mm` (INTEGER nullable, migration 0107) = cạnh máy KẸP TỜ GIẤY (~8–12mm) → bình bài trừ vào chiều DÀI tờ in. (Ô cũ "Nhíp kẽm" `gripper_mm` = mép nhíp trên BẢN KẼM ~44mm đã GỠ ở mg `0228` — engine bình bài chưa từng đọc nó, giữ lại chỉ gây nhầm với nhíp giấy.) `le_hong_mm` trừ MỖI BÊN chiều rộng, `duoi_thang_mau_mm` trừ chiều dài; cả hai đã có cột từ trước nhưng tới migration 0107 mới có ô nhập trên UI. **Ba cột tốc độ — chỉ MỘT cột chảy vào tính toán:** `toc_do` = tốc độ TRUNG BÌNH, là số duy nhất Tính giá / Lệnh SX / Xếp lịch đọc (tên cột giữ nguyên dù nhãn UI đã đổi thành "Tốc độ trung bình"); `toc_do_min` / `toc_do_max` (NUMERIC(12,2) nullable, migration 0152) là dải năng lực, khai ở hai ô "Tốc độ tối thiểu / tối đa" cùng nhóm "Tốc độ & Vận hành" của form Máy (bày lại 08/09/2026 sau 4 ngày ẩn). Chúng KHÔNG dời điểm đặt lịch — thanh Gantt và mọi con số cam kết vẫn theo `toc_do` — nhưng có ba nơi ĐỌC để ra khoảng nhanh–chậm: `lsxBuoc.ts` (băng "Biên độ tốc độ máy" ở drawer bước lệnh), `lsx_service.thoi_luong_buoc` (`chiem_may_phut_min`/`_max`) và `xep_lich_service` (râu Gantt). Vì thế đừng ẩn ô đi nữa: engine vẫn đọc thì người khai nhìn thấy một khoảng "130h — 312h" mà không có cửa nào sửa. Chưa khai ⇒ ba mức bằng nhau, KHÔNG bịa min=max=TB. Service chặn `min > max`, `min > toc_do`, `max < toc_do` (mã `E-MAY-SPEED-RANGE`). Cho lịch CHẠY bằng khoảng [sớm–muộn] vẫn là việc viết lại lõi xếp lịch, không phải chỉ đọc thêm hai cột này. **`don_vi_toc_do`** (VARCHAR(32) sau migration 0153, trước là 16) mang mã dạng `<đơn vị đếm>_gio` **suy ra từ danh mục `don_vi_do`** — chủ tự thêm/xoá đơn vị ở màn "Đơn vị & quy đổi" là danh sách chọn tự đổi theo, không có bảng riêng. `don_vi_do.ma` rộng 24 nên mã ở đây tới 28 ký tự; SQLite không ép độ dài (test không bắt được) nhưng Postgres thật thì lỗi lúc lưu — đó là lý do nới cột. Lệnh SX so khớp mã này với thứ công đoạn ĐẾM (`_DV_VAO_SANG_NS`); lệch là bỏ qua tốc độ trong im lặng, nên **đừng đổi quy ước đặt mã**. 🔴 **`cong_thuc_luong` GỠ 06/09/2026 (mg `0274`)** — ô "Cách đo lượng theo đơn vị tốc độ" của máy. Cách đo nay khai theo CẶP ở `cong_doan_may.cong_thuc_gio`: treo ở máy thì mọi công đoạn chạy máy đó dùng chung một cách đo, trong khi cùng một máy in, công đoạn khổ 79×109 và khổ 11×11 đo khác nhau. Vẫn đọc SỐNG, vẫn ở bậc ⓿ của `LsxService._sl_theo_don_vi`, chỉ đổi chỗ khai.
...
**Purpose:** danh mục công đoạn động, trung tính với cách thực hiện. Công đoạn chỉ khai tổ/phòng ban phụ trách, đơn vị, setup, bù hao, máy làm được và VẬT TƯ định mức (`cong_doan_vat_tu`, mg `0316`); lựa chọn `may|to|thue_ngoai` cùng máy cụ thể thuộc bước KHSX. Tổ phụ trách là DANH SÁCH ở bảng nối `cong_doan_to` (18/09/2026, mg `0312`).
...
`0316`). Khai ở tab **"Vật tư"** của drawer Công đoạn — nhiều dòng, mỗi dòng một công thức. Đến lệnh
sản xuất, bước trỏ công đoạn nào là vật tư của công đoạn đó tự bung vào khối "Vật tư cần dùng"
(`lsx_cong_doan_vat_tu.tu_dong = true`).

Thay `cong_doan_dau_viec_vat_tu`: cùng nội dung, chỉ đổi chỗ neo từ ĐẦU VIỆC của tổ lên chính
CÔNG ĐOẠN. Chủ xưởng 18/09/2026: *"vật tư được bung theo công đoạn"*. Công đoạn là CÔNG NGHỆ, công
việc khoán là VIỆC CỦA TỔ — bảng giao điểm giữa hai thứ đó (`cong_doan_dau_viec`) gỡ cùng ngày.
...
lại được. Đo trên DB dev 18/09/2026: 0 cặp trùng ⇒ thực tế là chép 1-1. Phải chạy TRƯỚC `0320`.

**Tất cả cột:** `id`, `cong_doan_id`, `vat_tu_id`, `thu_tu`, `cong_thuc_luong`.

### `cong_doan_dau_viec`

> 🔴 **BẢNG ĐÃ DROP 18/09/2026 (mg `0320`)** — chủ xưởng chỉ vào đúng bảng "Đầu việc và định mức
> của tổ" trên màn Công đoạn: *"là tôi kêu bỏ cái này này"*. Vật tư đã chuyển lên
> `cong_doan_vat_tu` (mg `0316`), công thức khoán về `piece_rates.cong_thuc_khoan` (mg `0317`) —
> cả hai chạy TRƯỚC khi drop. MẤT không khôi phục được: năng suất người/giờ (min/max), kíp chuẩn,
> đơn vị năng suất, cách đo giờ của từng cặp (công đoạn × đầu việc). Bước TỔ nay gõ
> `lsx_cong_doan.so_gio_ke_hoach`; thợ chọn công việc khoán LÚC GHI MẺ (`san_xuat_batch.piece_rate_id`).
> Mục dưới giữ để tra lịch sử.
...
**Định mức nhân lực — MỘT số duy nhất (mg `0270`, 06/09/2026):** `so_nguoi_tieu_chuan` là kíp chuẩn của công đoạn: số điền sẵn vào bước lệnh cho **mọi** loại bước (máy · tổ · thuê ngoài), và là số chia trong công thức thời lượng bước Tổ (năng suất khoán khai theo đầu người). Sửa đè được tại từng bước; Service chỉ kiểm `≥ 1`.

🔴 **`so_nguoi_toi_thieu` (mg 0160) và `so_nguoi_toi_da` GỠ cùng mg `0270`.** Tối đa chỉ là trần tính thời gian kèm cảnh báo mềm; tối thiểu chỉ có răng khi ai đó chịu khai ≥ 2, mà mặc định 1 thì nó im sẵn — hai mốc đó bắt người khai điền ba ô cho một câu hỏi. Mất theo: chặn "thiếu người" của bàn Xung đột và cảnh báo "vượt trần".
...
> 🔴 **BẢNG ĐÃ DROP 18/09/2026 (mg `0320`)** — dữ liệu chép lên `cong_doan_vat_tu` ở mg `0316`
> (neo đổi từ đầu việc của tổ lên chính công đoạn). Mục dưới giữ để tra lịch sử. ⚠️ Nửa sau mục
> này (đơn vị vào/ra, `size_tiers`, `khoan_ghi_theo`, `kieu_bu_hao`) thực ra mô tả cột của
> `cong_doan` — trôi xuống đây từ trước, vẫn còn đúng.
...
Ô Đơn vị đầu vào / đầu ra là **menu ĐÓNG đúng 5 chặng** dòng giấy (`to_nguyen · to · con · tay · cai` — `models/don_vi_do.TRAM_DONG_GIAY`), KHÔNG trỏ vào danh mục Đơn vị & quy đổi. Hai ca hợp lệ (`cong_doan_service`, chặn `[E-CD-DONVI]`):
- **cùng để trống** — bước NGOÀI dòng giấy (ghi kẽm, đóng thùng), không dính chuỗi bù hao của giấy. Đơn vị + số của nó do người lập lệnh tự khai ở bước (`lsx_service.tu_khai_don_vi`; công thức sản lượng ra ở danh mục gỡ 18/09/2026, mg `0324`). Đây là một CÂU TRẢ LỜI, không phải "chưa khai".
- **hai đầu đều là chặng** — bước TRÊN dòng giấy, phải đúng chiều: cùng chặng, hoặc một nhịp trong `CAU_TRAM` (`to_nguyen→to` · `to→con` · `con→cai` · `to→tay` · `tay→cai` · `to→cai`). Nhảy cóc `to_nguyen→cai` bị chặn.

Ca **một đầu trống một đầu có** và ca **mã ngoài 5 chặng** đều bị chặn. Ca một-chân-trong-một-chân-ngoài (`cai → thung`, đóng gói) vì thế cũng không khai được: hệ số của cặp đó là sức chứa từng đơn, chưa có chỗ khai → cho qua là engine ăn hệ số 1 trong im lặng.

🔴 **Đã gỡ 06/09/2026:** lớp trung gian `don_vi_do.tram_dong_giay`. Giữa 11/08 và 06/09, bước khai đơn vị TỰ DO từ danh mục (`bai → kem`) và câu "có nằm trên dòng giấy không" hỏi cờ trạm trên danh mục đơn vị. Cờ đó chỉ cho phép ĐỔI TÊN một chặng chứ không thêm được chặng thứ 6 (`CAU_TRAM` + `_he_so_cau` đóng cứng trong code), đổi lại bắt người khai đơn vị — việc của kho và mua hàng — phải hiểu dòng giấy. Migration `0273` đưa mọi cặp đơn vị ngoài 5 chặng (`kem → kem`, `m2 → bai`) của `cong_doan` · `lsx_cong_doan` · `bai_ghep_cong_doan` về NULL; số lượng không đổi vì `_he_so_ngoai_dong(None, None)` trả đúng hệ số 1,0 mà cặp `kem → kem` đang cho.

🔴 **Đã gỡ 11/08/2026:** `CAP_DON_VI_HOP_LE` (6 cặp liệt kê theo MÃ). Nay chỉ còn `CAU_TRAM` liệt kê theo **CHẶNG**. Hệ số mỗi nhịp vẫn ở `lsx_service._he_so_cau` (lấy từ quy cách lệnh: bình bài · số mảnh xả · số tay), KHÔNG khai vào bảng cặp `don_vi_quy_doi` — khai hai nơi là hai số giấy đá nhau trên cùng một lệnh.

**NULL = bước KHÔNG CHẠM GIẤY** (ghi kẽm, đóng thùng) → loại khỏi dòng giấy, bù hao của nó không cộng vào số giấy phải mua. `lsx_service._don_vi_theo_buoc` trả `(None, None, 1.0)`. SL vào/ra của bước KHÔNG tự tính (`buoc_ngoai_dong` gỡ 18/09/2026, mg `0324`): người lập lệnh tự khai đơn vị + số ở bước, không khai thì đứng ở 0.
...
**Purpose:** các **TỔ phụ trách một công đoạn** (18/09/2026, mg `0312`) — bảng nối `cong_doan` ↔
`departments`. "Cán màng mờ" do tổ Cán lẫn tổ Thành phẩm làm. Khai ở ô "Phòng ban / Tổ phụ trách"
(chọn nhiều) của drawer Công đoạn; thân POST/PUT `/api/cong-doan` mang `department_ids` (vắng/`null` =
giữ nguyên). Bước lệnh (`lsx_cong_doan.department_id`) và bước chung của bài ghép
(`bai_ghep_cong_doan.department_id`) vẫn giữ MỘT tổ — người kế hoạch CHỌN trong danh sách này, server
chặn tổ ngoài danh sách khi tổ hoặc công đoạn của bước vừa đổi; tổ ĐẦU danh sách (`thu_tu` nhỏ nhất)
là mặc định lúc tạo lệnh / đổi công đoạn. Định mức đầu việc của công đoạn hợp lệ khi đầu việc làm được
ở ÍT NHẤT MỘT tổ trong danh sách.

| Column | Type | Key | Null | Default | Meaning |
|---|---|---|---|---|---|
| `cong_doan_id` | `Integer` | **PK**, **FK→cong_doan.id** | no | — | Công đoạn; `ON DELETE CASCADE` + ORM `delete-orphan` (bỏ tổ khỏi danh sách là xoá dòng). |
| `department_id` | `Integer` | **PK**, **IX** | no | — | Tổ (ref `departments.id`, soft-ref không FK cứng). Tổ bị xoá khỏi cây tổ chức thì dòng ở lại, form đánh dấu để người khai tự gỡ. |
| `thu_tu` | `Integer` | — | no | `0` | Thứ tự người khai chọn — tổ đầu là tổ mặc định của bước lệnh. |

### `cong_doan_may`

**Purpose:** MÁY chạy được một công đoạn, kèm **cách đo giờ** và **cách tính giá** của riêng cặp (công đoạn, máy) — mg `0271`, 06/09/2026.

**Tất cả cột:** `id`, `cong_doan_id`, `may_id`, `cong_thuc_gio`, `cong_thuc_gia`, `thu_tu`.

**Vì sao là bảng nối chứ không phải cột trên máy hay trên công đoạn:** cả hai con số đều là giao của VIỆC × MÁY. `cong_thuc_gio` treo ở máy (`may_thiet_bi.cong_thuc_luong`, gỡ ở mg `0274`) thì mọi công đoạn chạy máy đó dùng chung một cách đo — trong khi cùng một máy in, công đoạn khổ 79×109 và khổ 11×11 đo khác nhau. `cong_thuc_gia` treo ở công đoạn (`cong_doan.cong_thuc_gia`) thì mọi máy dùng chung một đơn giá — trong khi máy 5 màu khổ lớn và máy 2 màu khổ nhỏ có giá khác nhau.

`cong_doan_id` → FK `cong_doan.id` ON DELETE CASCADE. `may_id` là **soft-ref** tới `may_thiet_bi.id` (cùng lối `piece_rate_id`/`vat_tu_id` ở hai bảng con kia); `cong_doan_service` chặn id không tồn tại hoặc máy đã ngừng dùng. UNIQUE `(cong_doan_id, may_id)` — `uq_cd_may`.

`cong_thuc_gio` (TEXT nullable): ra **LƯỢNG theo đơn vị TỐC ĐỘ của máy**, KHÔNG ra giờ — engine vẫn tự chia tốc độ. `LsxService.sl_tinh_cua_buoc` đọc nó ở BẬC 0, trước cầu quy đổi; đọc **SỐNG** (không ghim vào bước) vì đổi máy là đổi cách đo. Trống = lùi về cầu quy đổi. ⚠️ ĐỪNG nhân `so_luot_chay` vào đây: `thoi_luong_buoc` đã tự nhân số lượt vào nhánh máy (`SL ÷ tốc độ × lượt`), viết thêm là đếm hai lần.

`cong_thuc_gia` (TEXT nullable): ra **TIỀN**, và **GHI ĐÈ** `cong_doan.cong_thuc_gia` khi phiếu tính giá có chọn đúng máy này (`tinh_gia_service._cong_doan_to_dict`). Chỉ có nghĩa với công đoạn nhóm `print`: `phieu_thanh_phan.may_id` là chỗ DUY NHẤT phiếu tính giá chọn máy, và nó nằm trong khối In. Trống = dùng công thức chung của công đoạn — cặp có dòng mà ô trống KHÔNG làm phiếu ra 0đ.

**KHÔNG backfill lúc tạo bảng:** `nhom_may_cho_phep` thường phủ hàng chục máy, sinh bằng ấy dòng rỗng chỉ để người dùng phải xoá bớt. Bảng ra đời RỖNG.

**Quan hệ với `cong_doan.nhom_may_cho_phep`:** hàng tick nhóm nay chỉ là **BỘ LỌC** để chọn máy cho bảng này. Luật chặn gán máy ở bước (`BaiGhepService.may_ngoai_cong_doan`) đọc danh sách máy này TRƯỚC; công đoạn chưa khai máy nào thì mới lùi về luật nhóm.
...
**Tất cả cột:** `id`, `ma`, `ten`, `ho`, `he_so_goc`, `hieu_luc_tu`, `ghi_chu`, `active`, `dung_lam_toc_do`, `tram_dong_giay`, `created_at`, `updated_at`.

🔴 **`cong_thuc` (CÁCH ĐO của đơn vị, mg `0192`) ĐÃ GỠ 17/08/2026 — mg `0215` DROP cột.** Bảng này nay trả lời đúng HAI câu: đơn vị nào có, và đổi qua lại thế nào (cặp `don_vi_quy_doi`, hệ số cố định).

**Vì sao gỡ:** cách đo treo ở ĐƠN VỊ là câu trả lời **dùng chung** cho mọi ai đếm bằng đơn vị đó, trong khi câu hỏi thật luôn thuộc về một cái CỤ THỂ — keo và mực cùng đo `kg` mà tiêu hao khác hẳn, hai máy cùng đo `to_gio` mà đếm lượt khác nhau. Né bằng cách đẻ `kg_keo`/`kg_giay_to_in` thì kho và mua hàng phải nhìn mấy cái tên đó thay vì `kg` thật.

**Số đo trước khi xoá** (DB dev, 17/08/2026): 3/20 đơn vị mang công thức (`bai`, `m2`, `kg` — cả ba là công thức gõ thử). Nơi dựa vào: 4 vật tư cho lượng BOM · 3 đầu việc khoán cho tiền khoán · 1 công đoạn (`CD-0001 Ghi kẽm CTP`) cho sản lượng. Chủ chốt "gỡ hết, không chuyển gì": chỗ nào cần thì khai lại ở ô của chính nó.

⚠️ **BỐN ô công thức LƯỢNG trong hệ (thay chỗ cột vừa gỡ) + ô TIỀN:**
...
Tất cả dùng chung bộ chip của `bien_cong_thuc`, nhưng kết quả khác nghĩa hẳn.

**`tram_dong_giay`** (VARCHAR(12) NULL, migration `0186`): 🔴 **CỘT CHẾT 06/09/2026** — không nơi nào đọc, không màn nào khai, giữ lại để dữ liệu cũ không mất (cùng kiểu `he_so_goc` và `dung_lam_toc_do`). Nó từng nói đơn vị này đứng ở chặng nào trên dòng giấy, để công đoạn khai đơn vị tự do rồi engine tra ngược ra chặng. Nay ô Đơn vị vào/ra của công đoạn là menu đóng đúng 5 chặng nên không cần lớp dịch nữa — xem `services/dong_giay.py` và mục `cong_doan.don_vi_vao`.
...
**Purpose:** KHO DỤNG CỤ của xưởng — **khuôn bế, khuôn ép nhũ và khung lụa** (nhan đề màn là "Khuôn" từ 18/09/2026 — trước đó "Khuôn & khung" từ 04/09/2026, "Khuôn" từ 16/08/2026; tên bảng + module quyền vẫn `khuon_be`, đổi là mọi vai mất sạch quyền màn này). Mỗi con dao làm riêng cho 1 ấn phẩm; đơn lặp lại thì lôi dao cũ ra dùng. Khai TAY: `ma` (KB-#### sinh ngầm) / `ten` (tên ấn phẩm) / `khach_hang_id` / `loai` / `so_ke` (vị trí lưu — thợ đọc ô này để đi lấy) / `tinh_trang` / `ghi_chu`. Xóa mềm (`active=false`) giữ dấu vết.
...
**Tất cả cột:** `id`, `ma`, `ten`, `khach_hang_id`, `loai`, `so_ke`, `tinh_trang`, `ghi_chu`, `active`, `created_at`, `updated_at`.
...
- 🔴 `ngay_ve_du_kien` ("Ngày có khuôn (dự kiến)") **ĐÃ GỠ** (mg `0293`, 10/09/2026) — cùng đường với `ngay_lam_khuon` trên, nay là ô ngày CUỐI CÙNG của kho khuôn ra đi. Không phép tính nào đọc nó: cửa "Sẵn sàng lập kế hoạch" chỉ soi bước cần dao đã CHỌN dao chưa (`lsx_cong_doan.khuon_be_id`), xếp lịch/phát hành không đọc, không đâu trừ lùi lead-time theo nó như `stock_requests.ngay_can`; điểm chặn thật của luật "bế phải có khuôn mới làm" là ô tổ tích ĐÃ NHẬN KHUÔN (`san_xuat_cong_viec.khuon_nhan_luc`, chốt 04/09/2026). Nó chỉ là chữ bày ra — và là một ràng buộc CỨNG (`khuon_be_service._validate` chặn lưu khi `dang_dat_lam` mà bỏ trống), nên ai cũng phải bịa một ngày rồi không quay lại sửa. `tinh_trang='dang_dat_lam'` GIỮ NGUYÊN: đó mới là câu có người chịu trách nhiệm cập nhật.
  ⚠️ Ảnh chụp CŨ trong `san_xuat_cong_viec.khuon_json` còn khoá `ngay_ve_du_kien` — JSON đóng băng lúc phát hành, không phải cột, và không còn code nào đọc.
...
- **Người báo (`nguoi_bao_id` + `nguoi_bao_ten`) do SERVER chốt lúc tạo, KHÔNG gõ, KHÔNG sửa**
  (14/09/2026, bỏ ô chữ "nhập hộ"): phiếu sinh từ yêu cầu chép TÊN người gửi yêu cầu (id để NULL);
  phiếu tổ kỹ thuật tự lập lấy tài khoản đang lập — tên `users.name`, id là `employees.id` nối với
  tài khoản đó (chưa nối thì NULL). Hai khoá không có trong `SuaChuaIn`/`SuaChuaPatch` lẫn
  `ASSIGNABLE_SUA_CHUA`. Thợ thấy máy hỏng thì tự báo ("Báo sự cố" ở bàn tổ / "Báo máy hỏng") để
  tên đi theo tài khoản của họ. Tên snapshot để nhân viên nghỉ việc vẫn tra được.
- **`may_id` chốt lúc tạo, KHÔNG sửa** (14/09/2026): phiếu sinh từ yêu cầu lấy máy của yêu cầu
...
**Purpose:** một lần người **ngoài tổ kỹ thuật** báo máy hỏng (20/08/2026). Tổ sửa chữa tiếp nhận
→ sinh `ky_thuat_sua_chua`, hoặc từ chối kèm lý do. Bảng mới → `create_all` tự dựng, không
migration schema; **mg 0225** chỉ thêm HÀNG dữ liệu `modules('yeu_cau_sua_chua', 'Báo máy hỏng')`
để ma trận phân quyền có ô mà tick.

Vì sao KHÔNG cho người ngoài ghi thẳng vào `ky_thuat_sua_chua`: phiếu là sổ công việc của tổ kỹ
thuật (mã SC chạy liên tục, mức độ là kết luận nghề, đóng phiếu đòi ảnh chứng thực). Cho cả xưởng
mở phiếu là hàng chờ của thợ sửa lẫn với lời báo chưa ai xác nhận. Ngược lại, bắt người báo đi tìm
...
- **`nguoi_bao_id` = TÀI KHOẢN ĐANG ĐĂNG NHẬP, không phải ô chọn** — khác
  `ky_thuat_sua_chua.nguoi_bao_id` (soft → `employees.id`). ⚠️ Hai cột cùng tên nhưng **trỏ hai bảng khác nhau** ⇒ lúc chuyển yêu cầu thành phiếu
  KHÔNG được chép sang; chỉ chép `nguoi_bao_ten`. Tên + bộ phận snapshot để người nghỉ việc / đổi
  phòng rồi vẫn tra được.
- **`may_dung` (BOOLEAN NOT NULL DEFAULT FALSE) xếp hàng chờ, không phải `muc_do`.** Máy đang dừng
  hẳn là thứ người báo BIẾT CHẮC; mức độ chỉ là cảm nhận của họ và tổ sửa chữa đặt lại lúc tạo
  phiếu. Thứ tự danh sách cố định: chưa tiếp nhận → `may_dung` → `muc_do` → mới nhất, **không có
  tham số sắp xếp** — đây là hộp việc đến, không phải bảng tra cứu.
- **Không có đường XOÁ.** Không dùng thì `tu_choi` + `ly_do_tu_choi` (bắt buộc, ≤300 ký tự) — xoá
  lặng lẽ là người báo không bao giờ biết vì sao, và lần sau họ thôi không báo nữa.
- **`phieu_id` là sợi dây DUY NHẤT giữa hai bảng** (đã có index). Bảng phiếu KHÔNG mọc thêm cột
  `yeu_cau_id`: quan hệ 1-1 chỉ cần một sợi, và thêm cột vào bảng đang có dữ liệu thật thì phải
  viết migration cho DB live. Màn phiếu đọc NGƯỢC qua cột này (`repo.yeu_cau_map`) để hiện "sinh từ
  YC-0007 · Trần Văn Hải · Tổ in".
- Ảnh kèm yêu cầu dùng chung `ky_thuat_may_anh` với `loai_phieu="yeu_cau"`; lúc thành phiếu chúng
  **ĐỔI CHỦ** (update sang `sua_chua` + `giai_doan="truoc"`), không chép — xem mục bảng ảnh.
- Quyền: `yeu_cau_sua_chua` (create = gửi yêu cầu · read = xem hàng chờ · update = sửa yêu cầu của
  chính mình). Biến yêu cầu thành phiếu đòi `ky_thuat_may:create`, từ chối đòi `ky_thuat_may:update`
  — hai việc đó là quyết định của tổ sửa chữa. Khoá nằm trong `SCOPELESS_MODULES`: ai cũng THẤY hết
  yêu cầu đang chờ (để không báo trùng), còn "chỉ sửa của mình" chặn bằng `nguoi_bao_id` chứ không
  bằng scope.
...
(dinh_ky·dot_xuat), `ngay_ke_hoach`, `ngay_ke_hoach_goc`, `ly_do_doi`, `ly_do_huy`, `hang_muc` (JSON
`[{id,ten,xong,bo_qua,ly_do_bo_qua}]`), `nguoi_thuc_hien_id`, `nguoi_thuc_hien`, `trang_thai`
(cho_thuc_hien·hoan_thanh·da_huy), `ngay_hoan_thanh`, `hoan_thanh_boi`, `ghi_chu`,
`created_at`, `updated_at`.

- **BA trạng thái** (chủ chốt 12/08/2026 · bổ sung `da_huy` 20/08/2026): `cho_thuc_hien` →
  `hoan_thanh`, hoặc `cho_thuc_hien` → `da_huy` (hủy kèm lý do). Nấc `dang_thuc_hien` và cả bước
  "nhận việc" ĐÃ BỎ — bảo trì định kỳ làm xong trong một lượt, bắt bấm hai lần chẳng nói thêm được
  gì. **mg 0193** đẩy phiếu đang kẹt ở nấc cũ về `cho_thuc_hien` (trạng thái lưu bằng CHUỖI nên DB
  không tự chặn: không dọn thì phiếu mang giá trị không còn trong `TRANG_THAI_BAO_TRI`, mọi tab đều
  không khớp và nó biến mất khỏi màn hình mà không báo lỗi).
- **`da_huy` + `ly_do_huy`** (String(300), **mg 0224**) — thay chức năng "Dời lịch" đã gỡ khỏi UI
  bằng "Hủy phiếu kèm lý do". Chỉ hủy được phiếu đang `cho_thuc_hien`; hủy bắt buộc ghi lý do (ghi
  cả vào AuditLog). `da_huy` **KHÔNG** thuộc `TT_BT_DANG_MO` ⇒ không đếm badge, không tính "quá
  hạn", bị loại khỏi lịch, và chặn ticker sinh lại kỳ định kỳ vừa hủy. Mở lại phiếu (hủy nhầm) đưa
  về `cho_thuc_hien` và xoá `ly_do_huy` về NULL. Hai cột `ngay_ke_hoach_goc`/`ly_do_doi` GIỮ LẠI để
  phiếu cũ từng dời vẫn hiện "Đã dời", dù UI không còn tạo dữ liệu dời mới.
...
**Purpose:** ảnh minh chứng dùng chung cho **cả ba** loại chứng từ qua cặp (`loai_phieu`,
`phieu_id`) — một bảng chứ không ba, để mọi chỗ đọc/đếm/xoá ảnh chỉ viết một lần. File nằm dưới
subdir `ky-thuat-may/<loai_phieu>/`, phục vụ qua `/api/files`; quyền đọc tra theo ĐOẠN THỨ HAI của
khoá: `bao_tri` → `phieu_bao_tri`, `yeu_cau` → `yeu_cau_sua_chua` **hoặc** `ky_thuat_may`, còn lại
→ `ky_thuat_may`.

**Tất cả cột:** `id`, `loai_phieu` (sua_chua·bao_tri·yeu_cau), `phieu_id`, `giai_doan` (truoc·sau),
`file_name`, `file_url`, `file_type`, `uploaded_by`, `uploaded_at`.

- `loai_phieu="yeu_cau"` (20/08/2026): ảnh máy hỏng người báo chụp bằng điện thoại. Ép cứng
  `giai_doan="truoc"` — "ảnh chứng thực sau khi sửa" là chuyện của phiếu.
- **Lúc yêu cầu thành phiếu, ảnh CHUYỂN chủ chứ không chép**: `loai_phieu` → `sua_chua`, `phieu_id`
  → id phiếu mới, `giai_doan` → `truoc`, cùng một transaction với việc đổi trạng thái yêu cầu. Chép
  ra dòng thứ hai là hai dòng DB cùng trỏ MỘT khoá storage — gỡ ảnh ở một bên thì bên kia còn dòng
  mà tệp đã bay. ⚠️ **Khoá storage KHÔNG đổi** (vẫn mang đoạn `yeu_cau`, vì đổi khoá là phải chép
  tệp) ⇒ đó là lý do `files.py` mở đoạn `yeu_cau` cho cả hai ô quyền.
...
**Purpose:** 1 dòng công đoạn gia công sau in (finishing) của 1 thành phần — con của `phieu_thanh_phan` (`thanh_phan_id` FK thật, cascade xoá). Hoặc tính giá PHẲNG (`don_gia` > 0 × số lượng — `so_luong`=0 nghĩa dùng SL đặt của phiếu) hoặc dùng cấu hình công đoạn danh mục (`cong_doan_id`, soft FK, index `ix_phieu_thanh_pham_cong_doan_id` — migration `0301`) qua `routing_engine.compute_step_cost` với `so_mat`/`so_vi_tri`/`dien_tich`. `nha_cung_cap` → nhãn thuê ngoài. `bu_hao` cờ báo dòng có góp hao. (Không cột lợi nhuận — đây là giá vốn.)

**Tất cả cột:** `id`, `thanh_phan_id`, `thu_tu`, `cong_doan_id`, `ten`, `don_gia`, `so_luong`, `bu_hao`, `so_mat`, `so_vi_tri`, `dien_tich`, `nha_cung_cap`, `ghi_chu`, `phi_khuon`, `khuon_nguon`, `dai_khuon`, `rong_khuon`, `so_khuon`, `created_at`, `updated_at`.
...
**Purpose:** 1 dòng CHI PHÍ KHÁC của 1 sản phẩm tính giá — cặp (tên tự gõ, số tiền), khoản MỘT LẦN cho cả sản lượng; con của `phieu_thanh_phan` (`thanh_phan_id` FK thật, cascade xoá — migration `0304`). Chỗ hứng khoản chưa có danh mục nào nhận: làm kẽm ngoài, phí thiết kế, tiền mẫu. KHÔNG trỏ danh mục, KHÔNG công thức — máy chỉ cộng tiền và chép lại đúng cái tên người lập phiếu gõ. Engine (`thanh_phan_engine`) đẻ mỗi dòng > 0đ thành một dòng tiền của nhóm kết quả `chi_phi_khac` ⇒ CÓ cộng vào `gia_von_tp` (chịu markup ở Báo giá, bị chia theo sản lượng như `phi_giao_hang`/`phi_khuon`). Dòng 0đ vẫn lưu (để gõ tiếp) nhưng không thành dòng tiền. KHÔNG xuống màn xưởng: `lsx_service._tinh_dong` chỉ chép trường vô hướng vào `quy_cach_json`, danh sách này là `list` nên bị bỏ qua.

**Tất cả cột:** `id`, `thanh_phan_id`, `thu_tu`, `ten`, `so_tien`, `created_at`, `updated_at`.

- `ten` (`String(255)`): chữ TỰ DO, không có danh mục để chọn — thứ hay rơi vào đây đúng là thứ chưa ai lường trước để lập danh mục. Trống mà có tiền → engine gọi dòng đó là "Chi phí khác".
- `so_tien` (`Numeric(18,2)`): tổng tiền của khoản, MỘT LẦN cho cả sản lượng (không nhân số lượng).
...
| `giu_cho_bat` | `Boolean` → `BOOLEAN` | — | no | `false` | **CÔNG TẮC giữ chỗ vật tư** (mg `0208`). Bật = ĐĂNG KÝ giữ, không phải chụp một lần: giữ được bao nhiêu hay bấy nhiêu, hàng về sau thì tự nhặt thêm. Cần cờ RIÊNG chứ không suy từ "có dòng nào trong `vat_tu_giu_cho` không" — trạng thái *đã bật mà chưa giữ được gì* (kho trống, đang chờ mua) không có dòng nào để suy, mà đó chính là trạng thái phải nhớ. Xếp lịch chỉ mở khi giữ ĐỦ 100%. |
...
| `bat_buoc` | `Boolean` → `BOOLEAN` | — | no | `true` | LUÔN `true` từ 07/09/2026 (mg 0275): bước đã khai trong routing thì phải làm. Drawer không còn ô tick, `LsxCongDoanIn` không còn nhận field. |
| `so_luong_vao` | `Numeric(14,2)` | — | no | `0` | SL đầu vào bước. |
| `so_luong_ra` | `Numeric(14,2)` | — | no | `0` | SL đầu ra bước. |
| `don_vi_vao` | `String(24)` | — | no | `to` | Một trong 5 chặng dòng giấy (`TRAM_DONG_GIAY`) — kế thừa từ `cong_doan.don_vi_vao` khi bung lệnh. NULL = bước NGOÀI dòng giấy (mg `0186` nới từ 12; mg `0273` đưa mã ngoài 5 chặng về NULL). |
...
| `so_gio_ke_hoach` | `Numeric(8,2)` | — | no | `0` | **SỐ GIỜ KẾ HOẠCH của bước TỔ** (mg `0319`, 18/09/2026) — người lập kế hoạch GÕ TAY, nhận số lẻ (4,5 h). Thời lượng bước Tổ = `phat_sinh_phut + so_gio_ke_hoach × 60`. `0` = CHƯA KHAI và **KHÔNG cảnh báo gì** — chủ xưởng: *"bản chất nó là số giờ kế hoạch, nếu thiếu thì cứ để 0"*. Bước MÁY bỏ qua cột này (giờ vẫn từ `may_thiet_bi.toc_do` + `cong_doan_may.cong_thuc_gio`, đọc SỐNG). Không backfill từ kíp × năng suất cũ: "mấy người" không đổi ra "mấy giờ". |
...
`tu_dong` (BOOLEAN NOT NULL DEFAULT false, mg `0191`): **MÁY BUNG hay NGƯỜI KHAI.** `true` = dòng máy tự thêm theo CÔNG ĐOẠN của bước (danh sách lấy từ `cong_doan_vat_tu` từ 18/09/2026 — trước đó từ `cong_doan_dau_viec_vat_tu` theo đầu việc khoán chọn ở bước; số lượng thế quy cách lệnh vào `cong_thuc_luong`) ⇒ lần bung sau được **thay** bộ mới. `false` = người tự thêm, hoặc dòng máy bung nhưng người đã sửa số lượng ⇒ máy **CHỪA RA**, không ghi đè. Không có cờ này thì đổi công việc khoán một cái là mất sạch số người vừa chỉnh.
...
**Purpose:** GIỮ CHỖ vật tư — phần tồn kho đã có **chủ**, chưa xuất nhưng không ai khác lấy được. Chủ dự án chốt 17/08/2026: *lệnh phải giữ được vật tư thì mới được xếp lịch.*

Trước đó bảng cân đối **chỉ đọc**, tồn không thuộc về ai: lệnh A xếp lịch 22/8 dựa trên 60 kg giấy đang có, chiều hôm sau lệnh B lĩnh mất 50 kg, và lịch của A thành **lịch ma** mà không ai báo. Bảng này là chỗ tồn được đặt chỗ — và `tồn tự do = tồn − Σ đã giữ` mới là con số kho cho người khác lĩnh.
...
- `ck_giu_cho_so_duong` — `so_luong > 0`.

**Cố ý KHÔNG có UNIQUE trên (chủ thể, mặt hàng):** một chủ thể có thể giữ CÙNG một mặt hàng từ HAI nguồn — phần đã nằm trong kho và phần bám vào lô đang về (hai ngày về khác nhau thì hai dòng). Ép unique là ép gộp hai thứ có ràng buộc lịch khác hẳn nhau.

**Giữ theo (mặt hàng, SỐ LƯỢNG), KHÔNG đích danh lô** — chỉ lô sẽ phá nhập-trước-xuất-trước của kho: giữ lô cũ cho lệnh chạy tháng sau thì lô đó nằm ì, còn lệnh tuần này phải bóc lô mới. Kho cứ xuất theo thứ tự của kho; giữ chỗ chỉ ăn vào con số tồn tự do.

**Công tắc KHÔNG nằm ở đây** mà ở `lsx.giu_cho_bat` / `bai_ghep.giu_cho_bat` — xem lý do tại hai cột đó.
...
| `ten` | `String(255)` → `VARCHAR(255)` | — | no | — | Tên quản trị của bài (mg `0212`); bài cũ backfill `Bài ghép {ma}`. |
| `han_hoan_thanh_sx` | `Date` → `DATE` | — | yes | — | Hạn của bài; lúc tạo mặc định = hạn nhỏ nhất của thành viên, sau đó người dùng sửa độc lập. |
| `is_rush` | `Boolean` → `BOOLEAN` | — | no | `false` | Cờ gấp của bài; lúc tạo mặc định = có ít nhất một thành viên gấp, sau đó không tự đồng bộ lại. |
| `nguoi_phu_trach_id` | `Integer` | — | yes | — | Soft → `users.id`; lúc tạo mặc định là người thao tác, sau đó sửa độc lập. |
...
| `giu_cho_bat` | `Boolean` → `BOOLEAN` | — | no | `false` | **CÔNG TẮC giữ chỗ vật tư** (mg `0208`) — song song `lsx.giu_cho_bat`. Bài ghép là CHỦ THỂ giữ chỗ của các lệnh thành viên (lệnh đã ghép không giữ riêng, cùng luật với dòng nhu cầu ở bảng cân đối), nên công tắc nằm ở đây chứ không ở từng lệnh. Bài đang giữ chỗ thì **không thêm/rút thành viên, không phá bài** — ba việc đó đều làm đổi số tờ của cả bài. |
...
| `lsx_id` | `Integer` | FK→`lsx.id` (RESTRICT), U, IX | no | — | LSX thành viên — unique toàn cục (mg `0212`) chốt 1 LSX chỉ thuộc tối đa 1 bài; RESTRICT chặn xoá LSX đang ghép. |
...
- Primary key: `id`. Foreign keys: `bai_ghep_id` FK→`bai_ghep.id` (on delete CASCADE), `lsx_id` FK→`lsx.id` (on delete RESTRICT). Unique: `lsx_id` = `uq_bai_ghep_thanh_vien_lsx_id` (chống một LSX vào hai bài, kể cả request đồng thời); `(bai_ghep_id, lsx_id)` = `uq_bai_ghep_lsx` giữ tương thích. Indexes: `bai_ghep_id`, `lsx_id`.

**Relationships**

- Nhiều `bai_ghep_thanh_vien` thuộc một `bai_ghep`; mỗi thành viên trỏ một `lsx`. Guard "1 LSX ≤ 1 bài ghép" có cả service (`NOT EXISTS`) và unique DB.
...
| `bat_buoc` | `Boolean` | — | no | `true` | Bước bắt buộc (snapshot) — LUÔN `true`, chép từ bước gốc (mg 0275). |
...
| `so_gio_ke_hoach` | `Numeric(8,2)` | — | no | `0` | Số giờ kế hoạch của bước TỔ chung — mirror `lsx_cong_doan.so_gio_ke_hoach` (mg `0319`): gõ tay, số lẻ được, `0` = chưa khai, không cảnh báo. |
...
- Primary key: `id`. Foreign keys: `lsx_id` FK→`lsx.id` (on delete CASCADE), `bai_ghep_id` FK→`bai_ghep.id` (on delete CASCADE). Indexes: `lsx_id`, `lsx_cong_doan_id`, `bai_ghep_id`, `may_id`, `department_id`, `work_shift_id`, `goc_dong_id` (`ix_xep_lich_cong_doan_goc_dong_id`, mg `0253` — tra CẢ CỤM phân đoạn của một bước), tổ hợp `ix_xep_lich_may_thoigian` (`may_id`, `start_at`), tổ hợp `ix_xep_lich_start_trangthai` (`start_at`, `trang_thai`, mg `0287` — bàn làm việc cắt theo CỬA SỔ NGÀY và hỏi nhóm nháp `start_at IS NULL`, hai câu đều không có máy để bám). Index của `bai_ghep_cong_doan_id` chỉ có trên DB dựng bằng `create_all`: cột thêm bằng `ALTER TABLE` ở mg `0151` mà thiếu `CREATE INDEX`, mg `0287` vá lại cho DB cũ. Tổ hợp `ix_xep_lich_may_xong` (`may_id`, `finish_at`) và `ix_xep_lich_to_xong` (`department_id`, `finish_at`) — mg `0323`: hàng đèn Kế hoạch SX chỉ nạp việc cùng máy/tổ CHẠM khoảng giờ của trang lệnh (`finish_at >= lo AND start_at <= hi`), dẫn bằng `finish_at` để chỉ quét từ `lo` trở đi thay vì cả lịch sử của máy/tổ.
...
**Purpose:** Mốc BẮT ĐẦU do người điều độ đặt cho MỘT lệnh sản xuất (module Xếp lịch 3, RBAC `xep_lich_3`). Một dòng = một lệnh. Ngày kết thúc lệnh, mốc từng công đoạn và các đoạn máy chạy trong ca đều là số DẪN XUẤT — tính lúc đọc ở `services/xep_lich_3`, KHÔNG lưu cột. Bảng mới, có migration `0291`.

| Column       | Type (SQLAlchemy → SQLite / Postgres)                  | Key                     | Null | Default        | Meaning                                                                    |
| ------------ | ------------------------------------------------------ | ----------------------- | ---- | -------------- | -------------------------------------------------------------------------- |
| `id`         | `Integer` → `INTEGER` / `SERIAL`                       | **PK**                  | no   | auto           | Surrogate PK.                                                              |
| `lsx_id`     | `Integer` → `INTEGER`                                  | FK→`lsx.id` (CASCADE), **UNIQUE** | no | —    | Lệnh được xếp. UNIQUE vì màn ở cấp LỆNH: hai mốc cho một lệnh là mâu thuẫn. |
| `bat_dau_at` | `DateTime(timezone=True)` → `DATETIME` / `TIMESTAMPTZ` | —                       | no   | —              | **Thứ duy nhất người quyết.** Wall-clock naive (giờ nhà máy).              |
| `created_by` | `Integer` → `INTEGER`                                  | —                       | yes  | —              | Soft → `users.id` — ai đặt mốc lần đầu.                                    |
| `created_at` | `DateTime(timezone=True)`                              | —                       | no   | now (UTC)      | Khi tạo.                                                                   |
| `updated_at` | `DateTime(timezone=True)`                              | —                       | no   | now/onupdate   | Chốt chống ghi đè khi hai người cùng kéo (`expected_updated_at` → 409).    |

**Keys & indexes**

- Primary key: `id`. Foreign key: `lsx_id` → `lsx.id` (`ondelete=CASCADE`). Index: `ix_xep_lich_lenh_lsx` (`lsx_id`, UNIQUE).
...
## Thực hiện sản xuất — nhóm thành phẩm & gói phát hành (Giai đoạn 1)

> Lớp thực thi SAU khâu phát hành của Xếp lịch 2 (spec `docs/spec-thuc-hien-san-xuat.md`). 6 bảng dưới = nhóm "Nhóm và phát hành" (§19). Đều là bảng MỚI → `create_all` tự dựng, KHÔNG migration. Snapshot ĐÓNG BĂNG lúc phát hành: đổi cơ cấu/định mức/tổ ở danh mục sau đó KHÔNG đụng công việc đang chạy. Số dẫn xuất tính lúc đọc ở service, không cache cột.

### `san_xuat_nhom`

**Purpose:** nhóm thành phẩm, tự sinh từ `OrderLine.nhom` (§3.1). Dòng đơn không có `nhom` → một nhóm đơn lẻ. Kế hoạch KHÔNG tự ghép/tách nhóm; sai thì sửa từ Sale/đơn hàng trước phát hành.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` → `INTEGER` / `SERIAL` | **PK** | no | auto | Surrogate PK. |
| `ma` | `String(40)` | **U** | no | — | Mã nhóm tự sinh (`NTP…`). |
| `order_id` | `Integer` FK→`orders.id` | IX | no | — | Đơn hàng chứa nhóm. |
| `khoa` | `String(160)` | — | no | — | Định danh ổn định trong đơn: `nhom` nếu có, ngược lại `line:<order_line_id>`. |
...
- PK `id`; Unique `ma`; Unique(`order_id`,`khoa`) — một nhóm dẫn xuất ↔ một khoá trong đơn.

**Tất cả cột:** `id`, `ma`, `order_id`, `khoa`, `nhom_label`, `ten`, `than_chinh_lsx_id`, `trang_thai`, `version`, `created_at`, `updated_at`.

### `san_xuat_nhom_lsx`

**Purpose:** dòng thành viên nhóm — nhóm ↔ LSX. Mỗi LSX thuộc TỐI ĐA 1 nhóm (`lsx_id` unique). `la_than_chinh` đánh dấu LSX thân chính.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `nhom_id` | `Integer` FK→`san_xuat_nhom.id` | IX | no | — | Nhóm cha (cascade). |
| `lsx_id` | `Integer` FK→`lsx.id` | **U** | no | — | LSX thành viên (neo THẬT; mỗi LSX ≤ 1 nhóm). |
| `order_line_id` | `Integer` FK→`order_lines.id` | — | yes | — | Truy vết nguồn nhóm (OrderLine.nhom). |
| `la_than_chinh` | `Boolean` | — | no | `false` | Đúng một dòng/nhóm = thân chính sau bước ghép đầu. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |

**Tất cả cột:** `id`, `nhom_id`, `lsx_id`, `order_line_id`, `la_than_chinh`, `created_at`.

### `san_xuat_goi_phat_hanh`

**Purpose:** gói phát hành = một THÀNH PHẦN LIÊN THÔNG (§4.1). Phát hành NGUYÊN TỬ toàn bộ nhánh + Bài ghép dùng chung + các nhóm bị nối qua cùng Bài ghép.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `ma` | `String(40)` | **U** | no | — | Mã gói tự sinh (`GPH…`). |
| `trang_thai` | `String(24)` | — | no | `dang_phat_hanh` | `dang_phat_hanh`/`da_thu_hoi` (thu hồi chỉ khi CHƯA việc nào bắt đầu). |
| `version_hien_tai` | `Integer` | — | no | `1` | Trỏ tới `san_xuat_phien_ban.so` đang hiệu lực. |
| `version` | `Integer` | — | no | `1` | Chống bấm trùng. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |
| `updated_at` | `DateTime(timezone=True)` | — | no | now/onupdate | |

**Tất cả cột:** `id`, `ma`, `trang_thai`, `version_hien_tai`, `version`, `created_at`, `updated_at`.

### `san_xuat_phien_ban`

**Purpose:** phiên bản phát hành/cập nhật của một gói (§4.3). `so` tăng dần; giữ lịch sử ĐỦ, không xoá phiên bản cũ.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `goi_id` | `Integer` FK→`san_xuat_goi_phat_hanh.id` | IX | no | — | Gói cha (cascade). |
| `so` | `Integer` | — | no | — | Số phiên bản 1,2,3… |
| `loai` | `String(16)` | — | no | `phat_hanh` | `phat_hanh` (lần đầu) / `cap_nhat`. |
| `ly_do` | `String(500)` | — | yes | — | Lý do (bắt buộc khi cập nhật). |
| `phat_hanh_by_id` | `Integer` FK→`users.id` | — | yes | — | Người phát hành. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |

**Keys & indexes**

- PK `id`; Unique(`goi_id`,`so`).

**Tất cả cột:** `id`, `goi_id`, `so`, `loai`, `ly_do`, `phat_hanh_by_id`, `created_at`.

### `san_xuat_cong_viec`

**Purpose:** công việc (work item) — SNAPSHOT một công đoạn lúc phát hành + neo trạng thái thực thi (§4.2, §18). Neo LSX/Bài ghép bằng FK THẬT; công đoạn neo LỎNG (`step_key` + id, không FK — replace_routing tái sinh id). `phien_ban_so` cho biết snapshot thuộc phiên bản nào.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `goi_id` | `Integer` FK→`san_xuat_goi_phat_hanh.id` | IX | no | — | Gói cha. |
| `phien_ban_so` | `Integer` | — | no | `1` | Phiên bản snapshot. |
| `nhom_id` | `Integer` FK→`san_xuat_nhom.id` | IX | yes | — | Nhóm thành phẩm. |
| `lsx_id` | `Integer` FK→`lsx.id` | IX | yes | — | LSX (NULL cho công đoạn Bài ghép dùng chung). |
| `bai_ghep_id` | `Integer` FK→`bai_ghep.id` | IX | yes | — | Bài ghép dùng chung (một bản ghi thực hiện — §3.3). |
...
| `so_luong_ra` | `Numeric(18,3)` | — | yes | — | Định mức snapshot: lượng ra. |
| `don_vi_vao` | `String(40)` | — | yes | — | Đơn vị BẢN ĐỊA đầu vào của bước — ghi mẻ sản lượng · bàn giao · KCS · yêu cầu kho · phân bổ lương đều đọc cột này. Bước TRÊN dòng giấy chép `lsx_cong_doan.don_vi_vao` (mã chặng); bước NGOÀI dòng giấy chép đơn vị người lập lệnh tự khai ở bước (`kem` ⇒ ô Ghi mẻ hiện "Số lượng (bản kẽm)"), không khai thì trống — lối lấp bằng `cong_doan.don_vi_san_luong` (10/09/2026) gỡ 18/09/2026 cùng cột ấy (mg `0324`). KHÔNG dùng cột này để hỏi "bước ở chặng nào" — luật `tren_dong_giay` chỉ chấm được trên bản ghi KẾ HOẠCH, ở đây đọc `dinh_muc_json.ngoai_dong`. |
| `don_vi_ra` | `String(40)` | — | yes | — | Đơn vị BẢN ĐỊA đầu ra — cùng luật với `don_vi_vao` ngay trên. |
| `he_so_quy_doi` | `Numeric(18,6)` | — | yes | — | Hệ số quy đổi vào→ra. |
| `dinh_muc_json` | `JSON` | — | yes | — | Snapshot định mức chi tiết: `{so_nhan_cong_tieu_chuan, setup_phut, nang_suat, don_vi_nang_suat, chay_phut, chay_phut_min, chay_phut_max, phat_sinh_phut, ngoai_dong}`. `sl_dien_giai` (câu "Số bản kẽm = 4 bản kẽm") gỡ 18/09/2026 cùng công thức sản lượng ra (mg `0324`); dòng cũ còn khoá đó, không ai đọc. `chay_phut*` lấy từ engine `thoi_luong_buoc` (dải min/max = tốc độ tối đa/tối thiểu của máy; máy chưa khai dải ⇒ ba số bằng nhau) và **nhân tỉ lệ sản lượng của phân đoạn** — bước tách hai mẻ thì mỗi thẻ mang nửa giờ chạy. Trước ngày đó `chay_phut` đọc cột `lsx_cong_doan.chay_phut` (ô nhập đè, dormant từ 04/08/2026 nên luôn NULL) ⇒ bàn tổ không có con số giờ nào. `ngoai_dong` = bước NGOÀI dòng giấy — chụp cờ vì bước khai tay mang đơn vị riêng (`kem`) thì không màn nào suy ngược được nữa. |
| ~~`khoan_json`~~ | `JSON` | — | yes | — | 🔴 GỠ 18/09/2026 (mg `0321`) — bước lệnh thôi chọn đầu việc nên chẳng còn gì để đóng băng lúc phát hành; ảnh chụp nay ở MẺ (`san_xuat_batch.*_khoan_snapshot`, mg `0318`), chụp đúng lúc biết việc gì. Mô tả cũ: Snapshot cấu hình lương khoán (nội bộ bắt buộc hợp lệ — §4.2). Cùng hình dạng `lsx_cong_doan.khoan_json`, THÊM khoá `don_gia_hd` (08/09/2026) khi ô "Công thức tính tiền công" của bước RA THẲNG TIỀN (có chip `don_gia_khoan`): đơn giá **hiệu dụng** = tiền của bước ÷ `so_luong_ra`, tức cả công thức đã gộp vào một đơn giá trên đơn vị RA. Chốt lúc PHÁT HÀNH (`san_xuat/snapshot._SoPhatHanh.khoan_json`) vì công thức ăn `sl_vao`/`sl_ra` — hai số còn đổi suốt lúc lập kế hoạch. `don_gia` gốc vẫn giữ nguyên cạnh nó để đối chiếu ảnh chụp; **vắng khoá này = bước khai kiểu cũ**, tầng lương nhân `don_gia`. |
...
**Purpose:** ảnh chụp LỊCH CŨ của một công việc, ghi ngay TRƯỚC khi `phat_hanh_cap_nhat` đè lên (mg `0294`, 10/09/2026). `san_xuat_cong_viec` **sửa đè tại chỗ** rồi nâng `phien_ban_so` — docstring model trước đây hứa "đẻ dòng mới, giữ lịch sử" là SAI, đã sửa cùng đợt. Đè tại chỗ là cố ý (`cong_viec.id` bị `san_xuat_phu_thuoc`, phân công, hỗ trợ, batch, bàn giao, KCS trỏ tới), nên đây là chỗ DUY NHẤT còn bản cũ của lịch.

Chỉ chép ba thứ đổi được qua một lần cập nhật lịch: máy + hai mốc. Hành lý còn lại (đơn vị · định mức · quy cách · dặn dò) cũng bị tái chụp nhưng KHÔNG vào đây — bảng này trả lời "lịch đã đổi thế nào", chép cả snapshot là dựng bản sao thứ hai của bảng 40 cột rồi lệch ngay lần đầu ai thêm cột.

⚠️ **KHÔNG backfill được:** phiên bản có trước 10/09/2026 đã bị đè mất, không nguồn nào dựng lại. UI so sánh phải chịu được phiên bản cũ trống chứ không được đọc thành "không đổi gì".

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `goi_id` | `Integer` FK→`san_xuat_goi_phat_hanh.id` (CASCADE) | IX | no | — | Gói cha. |
| `phien_ban_so` | `Integer` | IX | no | — | Số của bản **CŨ** vừa bị đè (không phải số mới). |
| `cong_viec_id` | `Integer` FK→`san_xuat_cong_viec.id` (CASCADE) | IX | no | — | Công việc mà dòng này là ảnh chụp cũ của nó. |
| `may_id` | `Integer` soft→`may_thiet_bi.id` | — | yes | — | Máy CŨ. Khoá mềm, cùng quy ước `san_xuat_cong_viec.may_id`. |
| `du_kien_bat_dau` | `DateTime(timezone=True)` | — | yes | — | Mốc bắt đầu CŨ — thang LỊCH (giờ tường dán nhãn UTC), y như cột nguồn. |
| `du_kien_ket_thuc` | `DateTime(timezone=True)` | — | yes | — | Mốc kết thúc CŨ, cùng thang. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | Lúc chụp. |

**Keys & indexes**

- PK `id`; IX `goi_id`; IX `cong_viec_id`; IX `ix_sx_cv_lich_su_goi_ver` (`goi_id`,`phien_ban_so`) — đường đọc của màn so sánh phiên bản.

**Tất cả cột:** `id`, `goi_id`, `phien_ban_so`, `cong_viec_id`, `may_id`, `du_kien_bat_dau`, `du_kien_ket_thuc`, `created_at`.

### `san_xuat_phu_thuoc`

**Purpose:** cạnh phụ thuộc chéo giữa hai LSX cùng nhóm — SNAPSHOT bước ghép (§3.2). Chỉ tồn tại trong cùng một nhóm thành phẩm.
...
| `goi_id` | `Integer` FK→`san_xuat_goi_phat_hanh.id` | IX | no | — | Gói cha. |
| `phien_ban_so` | `Integer` | — | no | `1` | Phiên bản snapshot. |
| `nhom_id` | `Integer` FK→`san_xuat_nhom.id` | IX | no | — | Nhóm thành phẩm. |
| `nguon_cong_viec_id` | `Integer` FK→`san_xuat_cong_viec.id` | — | no | — | Công việc nguồn (LSX/công đoạn cấp đầu vào). |
| `dich_cong_viec_id` | `Integer` FK→`san_xuat_cong_viec.id` | — | no | — | Công việc đích = bước ghép. |
| `ty_le_ghep` | `Numeric(18,6)` | — | yes | — | Tỷ lệ ghép (gợi ý từ đơn/bài tính giá, sửa trước phát hành). |
| `don_vi_nguon` | `String(40)` | — | yes | — | Đơn vị nguồn. |
| `don_vi_dich` | `String(40)` | — | yes | — | Đơn vị đích. |
| `quy_tac_quy_doi` | `String(120)` | — | yes | — | Quy tắc quy đổi. |
| `so_luong_yeu_cau` | `Numeric(18,3)` | — | yes | — | Số lượng yêu cầu (sau quy đổi). |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |

**Tất cả cột:** `id`, `goi_id`, `phien_ban_so`, `nhom_id`, `nguon_cong_viec_id`, `dich_cong_viec_id`, `ty_le_ghep`, `don_vi_nguon`, `don_vi_dich`, `quy_tac_quy_doi`, `so_luong_yeu_cau`, `created_at`.

---

### `san_xuat_phan_cong`

**Purpose:** roster THỰC THI — ai được giao vào một công việc đã phát hành (§7.1). Bảng MỚI (`create_all`, không migration). Lần giao đầu = tổ tiếp nhận (§5.2). Rút người = `trang_thai=removed`, giữ lịch sử.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `cong_viec_id` | `Integer` FK→`san_xuat_cong_viec.id` (CASCADE) | IX | no | — | Công việc (work item đã đóng băng). |
| `employee_id` | `Integer` FK→`employees.id` (CASCADE) | IX | no | — | Người được giao (có thể KHÔNG có tài khoản). |
| `la_luong_khoan` | `Boolean` | — | no | `false` | Ảnh chụp lúc giao: người này thuộc chế độ lương khoán (§6). |
| `trang_thai` | `String(16)` | — | no | `active` | `active` \| `removed`. |
| `ly_do_rut` | `String(255)` | — | yes | — | Lý do rút khỏi công việc. |
| `version` | `Integer` | — | no | `1` | Chống bấm trùng. |
| `created_by` | `Integer` FK→`users.id` | — | yes | — | Tổ trưởng thực hiện thao tác. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |
...
**Purpose:** một PHIÊN chạy = khoảng công việc thực sự chạy (§7.2). Bảng MỚI (`create_all`). Bắt đầu mở phiên (`ket_thuc` NULL); Tạm dừng/Kết thúc đóng phiên. Một công việc nhiều phiên. Mốc lấy từ máy chủ, không backdate.
...
| ~~`ly_do_so_nguoi`~~ | `String(255)` | — | yes | — | 🔴 GỠ 18/09/2026 (mg `0321`) — câu giải thích "vì sao phiên này khác kíp chuẩn". Không còn kíp chuẩn thì không còn gì để lệch. |
| `ly_do` | `String(255)` | — | yes | — | Lý do tạm dừng / kết thúc trễ. |
| `version` | `Integer` | — | no | `1` | Chống bấm trùng. |
| `created_by` | `Integer` FK→`users.id` | — | yes | — | Người mở phiên. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |
| `updated_at` | `DateTime(timezone=True)` | — | no | now (UTC) | onupdate = now. |

**Tất cả cột:** `id`, `cong_viec_id`, `so_thu_tu`, `may_id`, `bat_dau`, `ket_thuc`, `loai_dong`, `ly_do_bat_dau_tre`, `ly_do`, `version`, `created_by`, `created_at`, `updated_at`.

---

### `san_xuat_khoang_tham_gia`

**Purpose:** khoảng một NGƯỜI tham gia một phiên chạy (§7.2, §7.3). Bảng MỚI (`create_all`). Mở khi phiên bắt đầu / thêm người; đóng khi tạm dừng·kết thúc·rút người. Phút thực tế cho lương TÍNH LÚC ĐỌC — bảng chỉ giữ khoảng thô.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `cong_viec_id` | `Integer` FK→`san_xuat_cong_viec.id` (CASCADE) | IX | no | — | Công việc. |
| `phien_chay_id` | `Integer` FK→`san_xuat_phien_chay.id` (CASCADE) | IX | no | — | Phiên chạy chứa khoảng. |
| `employee_id` | `Integer` FK→`employees.id` (CASCADE) | IX | no | — | Người tham gia. |
| `bat_dau` | `DateTime(timezone=True)` | — | no | — | Mốc vào (máy chủ). |
| `ket_thuc` | `DateTime(timezone=True)` | — | yes | — | Mốc ra; NULL = đang mở. Một người không hai khoảng mở chồng giờ (§7.1). |
| `version` | `Integer` | — | no | `1` | Chống bấm trùng. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |
| `updated_at` | `DateTime(timezone=True)` | — | no | now (UTC) | onupdate = now. |

**Tất cả cột:** `id`, `cong_viec_id`, `phien_chay_id`, `employee_id`, `bat_dau`, `ket_thuc`, `version`, `created_at`, `updated_at`.

> 🔴 `job_grade_id` · `output_coefficient` (ảnh chụp bậc/hệ số lúc mở khoảng, mg `0222`) **ĐÃ DROP**
> 17/09/2026 (mg `0305`) cùng danh mục bậc.

---

### `san_xuat_batch`

**Purpose:** một BATCH sản lượng của một công việc (§11.1). Bảng MỚI (`create_all`). Nhiều batch một phần trong cùng công đoạn. `tong = tot + hong` do service kiểm (dung sai làm tròn, không CHECK). Hỏng ghi kèm `mo_ta_loi` tự do, tuỳ chọn (danh mục lý do/lỗi ĐÃ GỠ — mg 0288). Người tham gia batch SUY LÚC ĐỌC từ khoảng tham gia giao cửa sổ `[bat_dau, ket_thuc]` (§12.1) — không lưu ở đây.
...
| `mo_ta_loi` | `String(500)` | — | yes | — | Mô tả lỗi bổ sung (không thay danh mục). |
| `ghi_chu` | `String(500)` | — | yes | — | Ghi chú. |
| `version` | `Integer` | — | no | `1` | Chống bấm trùng. |
| `created_by` | `Integer` FK→`users.id` | — | yes | — | Người ghi batch. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |
| `updated_at` | `DateTime(timezone=True)` | — | no | now (UTC) | onupdate = now. |
...
### `san_xuat_batch_phat_sinh`

**Purpose:** VIỆC PHÁT SINH thợ đã làm trong một mẻ, kèm số lượng (18/09/2026, mg `0318`; bảng MỚI ⇒ `create_all`). Ví dụ của chủ xưởng: mẻ 3.000 và "thay kẽm 2" — hai con số nằm CẠNH nhau. Việc phát sinh **KHÔNG BAO GIỜ cộng vào sản lượng**: không tiến độ, không KCS, không bàn giao, không nhập kho. Không có công thức, không có ô thành tiền: *"chỉ cần ghi nhận thay 2 bản kẽm thôi"*.
...
| `don_gia_snapshot` | `Numeric(14,2)` | — | yes | — | Ảnh chụp đơn giá — cùng luật băng "Danh mục đã đổi" với ba ô snapshot của mẻ. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |

**Tất cả cột:** `id`, `batch_id`, `phat_sinh_id`, `so_luong`, `ten_snapshot`, `don_vi_snapshot`, `don_gia_snapshot`, `created_at`.

---

### `san_xuat_batch_lot_vao`

**Purpose:** một LOT đầu vào đã dùng cho một batch (§10.3). Bảng MỚI (`create_all`), CHỈ-THÊM (không `version`): dựng quan hệ truy vết mẻ công đoạn trước → batch đầu ra. Nguồn DUY NHẤT là batch đầu ra công đoạn trước.

> 🔴 `nguon_loai` · `nguon_lot_id` (nguồn "lot BTP trong kho") **ĐÃ DROP** 17/09/2026 (mg `0308`) — chưa từng có màn nào ghi; dòng `nguon_loai <> 'batch'` bị xoá trước khi bỏ cột.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `batch_id` | `Integer` FK→`san_xuat_batch.id` (CASCADE) | IX | no | — | Batch đầu ra dùng lot này. |
| `nguon_batch_id` | `Integer` FK→`san_xuat_batch.id` (SET NULL) | IX | yes | — | Batch đầu ra công đoạn trước (service bắt buộc; SET NULL giữ vết khi batch nguồn bị gỡ). |
| `so_luong` | `Numeric(18,3)` | — | no | — | Lượng lot đưa vào batch. |
| `don_vi` | `String(24)` | — | no | — | Đơn vị (mặc định đơn vị vào của công việc). |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |

**Tất cả cột:** `id`, `batch_id`, `nguon_batch_id`, `so_luong`, `don_vi`, `created_at`.

---

### `san_xuat_ban_giao`

**Purpose:** BÀN GIAO sản lượng tốt sang công đoạn sau (§11.2). Bảng MỚI (`create_all`). MỘT số lượng thống nhất mỗi lần giao. Cùng tổ + cùng LSX → tạo thẳng `confirmed`; khác tổ/LSX → `proposed` rồi bên nhận `confirmed`. `khong_nhat_quan` bật khi điều chỉnh giảm dưới lượng công đoạn sau đã dùng (§11.3).

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `nguon_cong_viec_id` | `Integer` FK→`san_xuat_cong_viec.id` (CASCADE) | IX | no | — | Công đoạn giao đi. |
| `dich_cong_viec_id` | `Integer` FK→`san_xuat_cong_viec.id` (SET NULL) | IX | yes | — | Công đoạn nhận = một chặng sau theo routing — service BẮT BUỘC có. Bước cuối lệnh không bàn giao (thành phẩm vào kho qua KCS); "giao ra kho" (NULL) đã gỡ 17/09/2026. Nullable chỉ để SET NULL khi công việc đích bị gỡ. |
| `cung_to` | `Boolean` | — | no | `false` | Cùng tổ + cùng LSX (auto-confirm). |
| `so_luong` | `Numeric(18,3)` | — | no | — | Số lượng bàn giao thống nhất. |
| `don_vi` | `String(24)` | — | no | — | Đơn vị. |
| `trang_thai` | `String(16)` | — | no | `proposed` | `proposed` \| `confirmed` \| `adjusted`. |
| `khong_nhat_quan` | `Boolean` | — | no | `false` | Điều chỉnh xuống dưới lượng công đoạn sau đã dùng. |
| `de_xuat_by_id` | `Integer` FK→`users.id` | — | yes | — | Người đề xuất. |
| `de_xuat_luc` | `DateTime(timezone=True)` | — | no | now (UTC) | Mốc đề xuất. |
| `xac_nhan_by_id` | `Integer` FK→`users.id` | — | yes | — | Người xác nhận (bên nhận). |
| `xac_nhan_luc` | `DateTime(timezone=True)` | — | yes | — | Mốc xác nhận. |
| `version` | `Integer` | — | no | `1` | Chống bấm trùng. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |
| `updated_at` | `DateTime(timezone=True)` | — | no | now (UTC) | onupdate = now. |

**Tất cả cột:** `id`, `nguon_cong_viec_id`, `dich_cong_viec_id`, `cung_to`, `so_luong`, `don_vi`, `trang_thai`, `khong_nhat_quan`, `de_xuat_by_id`, `de_xuat_luc`, `xac_nhan_by_id`, `xac_nhan_luc`, `version`, `created_at`, `updated_at`.

---

### `san_xuat_ban_giao_dieu_chinh`

**Purpose:** một lần ĐIỀU CHỈNH số lượng bàn giao (§11.3). Bảng MỚI (`create_all`), CHỈ-THÊM (không `version`) — giữ lịch sử trước/sau, không xoá cứng. `khong_nhat_quan=true` nếu `so_luong_sau` thấp hơn lượng công đoạn sau đã tiêu thụ tại thời điểm điều chỉnh.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `ban_giao_id` | `Integer` FK→`san_xuat_ban_giao.id` (CASCADE) | IX | no | — | Bàn giao được điều chỉnh. |
| `so_luong_truoc` | `Numeric(18,3)` | — | no | — | Số lượng trước điều chỉnh. |
| `so_luong_sau` | `Numeric(18,3)` | — | no | — | Số lượng sau điều chỉnh. |
| `mo_ta` | `String(500)` | — | yes | — | Mô tả bổ sung. |
| `khong_nhat_quan` | `Boolean` | — | no | `false` | Cờ giảm dưới lượng đã dùng. |
| `created_by` | `Integer` FK→`users.id` | — | yes | — | Người điều chỉnh. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |

**Tất cả cột:** `id`, `ban_giao_id`, `so_luong_truoc`, `so_luong_sau`, `mo_ta`, `khong_nhat_quan`, `created_by`, `created_at`.

---

### `san_xuat_ban_giao_batch`

**Purpose:** MẺ nào đi theo LẦN BÀN GIAO nào (14/09/2026). Bảng MỚI (`create_all`). Form bàn giao liệt kê các mẻ chưa giao, tick mẻ nào thì mẻ đó gắn vào lần giao. `batch_id` UNIQUE — một mẻ đi theo đúng một lần giao, "mẻ chưa giao" là mẻ không có dòng ở đây. KHÔNG lưu số lượng theo mẻ: `san_xuat_ban_giao.so_luong` là số hai bên thống nhất (được sửa giảm khi đếm lệch). Bàn giao toả tự động của bài ghép không đi qua bảng này.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `ban_giao_id` | `Integer` FK→`san_xuat_ban_giao.id` (CASCADE) | IX | no | — | Lần bàn giao. |
| `batch_id` | `Integer` FK→`san_xuat_batch.id` (CASCADE) | **U**, **IX** | no | — | Mẻ sản lượng của công đoạn nguồn đi theo lần giao này. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |

**Tất cả cột:** `id`, `ban_giao_id`, `batch_id`, `created_at`.

---

### `san_xuat_vat_tu_nhan`

**Purpose:** TỔ XÁC NHẬN đã nhận vật tư của MỘT phiếu xuất đã ghi sổ (§10.1). Bảng MỚI (`create_all`). Xác nhận phiếu NGUYÊN TRẠNG — không đẻ con số "tổ nhận" đối nghịch "kho giao". `voucher_id` UNIQUE: một phiếu xác nhận một lần. Chỉ phiếu đã xác nhận mới coi là tồn khả dụng cho công đoạn.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `voucher_id` | `Integer` FK→`stock_vouchers.id` (CASCADE) | **U**, **IX** | no | — | Phiếu xuất đã ghi sổ được xác nhận. |
| `department_id` | `Integer` FK→`departments.id` | IX | no | — | Tổ nhận (node lá Khối SX). |
| `xac_nhan_by_id` | `Integer` FK→`users.id` | — | yes | — | Tổ trưởng xác nhận. |
| `xac_nhan_luc` | `DateTime(timezone=True)` | — | no | now (UTC) | Mốc xác nhận. |
| `ghi_chu` | `String(500)` | — | yes | — | Ghi chú. |
| `version` | `Integer` | — | no | `1` | Chống bấm trùng. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |
...
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `cong_viec_id` | `Integer` FK→`san_xuat_cong_viec.id` (CASCADE) | **IX** | no | — | Công đoạn đang thực hiện được hỗ trợ. |
| `employee_id` | `Integer` FK→`employees.id` (CASCADE) | **IX** | no | — | Người hỗ trợ. |
| `to_goc_id` | `Integer` FK→`departments.id` (SET NULL) | **IX** | yes | — | Tổ gốc của người hỗ trợ — nơi ghi nhận phần hỗ trợ (§9.2). |
| `to_thuc_hien_id` | `Integer` FK→`departments.id` (SET NULL) | **IX** | yes | — | Tổ đang thực hiện công đoạn (snapshot tổ của công việc). |
| `ngay_lam_viec` | `Date` | **IX** | no | — | Ngày làm việc — phần hỗ trợ thuộc ngày này, KHÔNG chuyển sang ngày hoàn thành (§9.2). |
| ~~`ty_le_phan_tram`~~ | — | — | — | — | 🔴 DROP 18/09/2026 (mg `0322`) — tỷ lệ chỉ sinh ra để CHIA sản lượng, tầng chia gỡ hẳn. Phiếu hỗ trợ nay chỉ là vết "ai sang giúp tổ nào, ngày nào" + hai bên xác nhận. |
| `trang_thai` | `String(16)` | — | no | `pending_both` | `pending_both` \| `confirmed` \| `cancelled`. |
| `mo_ta` | `String(500)` | — | yes | — | Ghi chú đề xuất. |
| `de_xuat_by_id` | `Integer` FK→`users.id` | — | yes | — | Người đề xuất thỏa thuận. |
| `xac_nhan_goc_by_id` | `Integer` FK→`users.id` | — | yes | — | Tổ trưởng tổ gốc xác nhận. |
| `xac_nhan_goc_luc` | `DateTime(timezone=True)` | — | yes | — | Mốc tổ gốc xác nhận. |
| `xac_nhan_thuc_hien_by_id` | `Integer` FK→`users.id` | — | yes | — | Tổ trưởng tổ thực hiện xác nhận. |
| `xac_nhan_thuc_hien_luc` | `DateTime(timezone=True)` | — | yes | — | Mốc tổ thực hiện xác nhận. |
| `huy_by_id` | `Integer` FK→`users.id` | — | yes | — | Người huỷ. |
| `huy_luc` | `DateTime(timezone=True)` | — | yes | — | Mốc huỷ (vd lịch chưa chạy bị phát hành lại §9.2). |
| `ly_do_huy` | `String(500)` | — | yes | — | Lý do huỷ. |
| `version` | `Integer` | — | no | `1` | Chống bấm trùng. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |
| `updated_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |

**Tất cả cột:** `id`, `cong_viec_id`, `employee_id`, `to_goc_id`, `to_thuc_hien_id`, `ngay_lam_viec`, `trang_thai`, `mo_ta`, `de_xuat_by_id`, `xac_nhan_goc_by_id`, `xac_nhan_goc_luc`, `xac_nhan_thuc_hien_by_id`, `xac_nhan_thuc_hien_luc`, `huy_by_id`, `huy_luc`, `ly_do_huy`, `version`, `created_at`, `updated_at`.

---

### `san_xuat_phan_bo`

> 🔴 **BẢNG ĐÃ DROP 18/09/2026 (mg `0322`)** — xoá TRỌN tầng chia sản lượng cho từng người. Chủ xưởng: *"mẻ đó 3 người ghi sản lượng 3000 và thay kẽm số lượng 2, thì ghi nhận thế thôi đừng có chia bất cứ gì"* · *"tôi sẽ có cách chia khi sang kế toán sau"*. Mục dưới giữ để tra lịch sử.
...
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `batch_id` | `Integer` FK→`san_xuat_batch.id` (CASCADE) | **U**, **IX** | no | — | Batch được phân bổ (một batch một phân bổ). |
| `cong_viec_id` | `Integer` FK→`san_xuat_cong_viec.id` (CASCADE) | **IX** | no | — | Công đoạn (denormalize từ batch). |
| `ngay` | `Date` | **IX** | no | — | Ngày của batch (suy kỳ lương). |
| `ky_nam` | `Integer` | — | no | — | Năm kỳ lương của batch. |
...
| `chot_by_id` | `Integer` FK→`users.id` | — | yes | — | Tổ trưởng chốt. |
| `chot_luc` | `DateTime(timezone=True)` | — | yes | — | Mốc chốt. |
| `mo_lai_by_id` | `Integer` FK→`users.id` | — | yes | — | Người mở lại. |
| `mo_lai_luc` | `DateTime(timezone=True)` | — | yes | — | Mốc mở lại. |
| `version` | `Integer` | — | no | `1` | Chống bấm trùng. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |
| `updated_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |
...
### `san_xuat_phan_bo_dong`

> 🔴 **BẢNG ĐÃ DROP 18/09/2026 (mg `0322`)** — xoá TRỌN tầng chia sản lượng cho từng người. Chủ xưởng: *"mẻ đó 3 người ghi sản lượng 3000 và thay kẽm số lượng 2, thì ghi nhận thế thôi đừng có chia bất cứ gì"* · *"tôi sẽ có cách chia khi sang kế toán sau"*. Mục dưới giữ để tra lịch sử.

**Purpose:** DÒNG phân bổ theo NGƯỜI (§12.2, Giai đoạn 4). Bảng MỚI (`create_all`), DẪN XUẤT — sinh lại toàn bộ mỗi lần tính/chốt, KHÔNG `version`. Giữ RIÊNG sản lượng trả lương và bản địa. `la_ho_tro=true` = phần người hỗ trợ (ghi cho tổ gốc, không chia theo phút); `false` = phần tổ thực hiện chia theo `trong_so=phut_thuc_te`. Cột `he_so_bac` ĐÃ DROP 17/09/2026 (mg `0305`) — bậc tay nghề gỡ hẳn.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `phan_bo_id` | `Integer` FK→`san_xuat_phan_bo.id` (CASCADE) | **IX** | no | — | Header phân bổ. |
| `employee_id` | `Integer` FK→`employees.id` (CASCADE) | **IX** | no | — | Người nhận phần. |
| `department_id` | `Integer` FK→`departments.id` (SET NULL) | **IX** | yes | — | Tổ ghi nhận (tổ gốc nếu hỗ trợ, tổ thực hiện nếu không). |
| `la_ho_tro` | `Boolean` | — | no | `false` | Dòng phần hỗ trợ hay phần tổ thực hiện. |
| `ho_tro_id` | `Integer` FK→`san_xuat_ho_tro.id` (SET NULL) | — | yes | — | Thỏa thuận hỗ trợ nguồn (chỉ dòng hỗ trợ). |
| `ngay` | `Date` | **IX** | no | — | Ngày làm việc của dòng (ngày batch / ngày thỏa thuận). |
| `so_luong_tra_luong` | `Numeric(18,3)` | — | no | `0` | Sản lượng TRẢ LƯƠNG phân cho người này. |
| `so_luong_ban_dia` | `Numeric(18,3)` | — | yes | — | Sản lượng bản địa tương ứng (giữ riêng §12.2). |
| `trong_so` | `Numeric(18,6)` | — | yes | — | Trọng số = phút thực tế hợp lệ (chỉ dòng tổ thực hiện). |
| `phut_thuc_te` | `Numeric(12,3)` | — | yes | — | Phút thực tế hợp lệ snapshot. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |

**Tất cả cột:** `id`, `phan_bo_id`, `employee_id`, `department_id`, `la_ho_tro`, `ho_tro_id`, `ngay`, `so_luong_tra_luong`, `so_luong_ban_dia`, `trong_so`, `phut_thuc_te`, `created_at`.

---

### `san_xuat_phan_bo_bu_tru`

> 🔴 **BẢNG ĐÃ DROP 18/09/2026 (mg `0322`)** — xoá TRỌN tầng chia sản lượng cho từng người. Chủ xưởng: *"mẻ đó 3 người ghi sản lượng 3000 và thay kẽm số lượng 2, thì ghi nhận thế thôi đừng có chia bất cứ gì"* · *"tôi sẽ có cách chia khi sang kế toán sau"*. Mục dưới giữ để tra lịch sử.

**Purpose:** DÒNG BÙ TRỪ sau khi kỳ lương ĐÃ KHÓA (§12.3, Giai đoạn 4). Bảng MỚI (`create_all`), CHỈ-THÊM (không `version`). Không sửa kỳ cũ — ghi chênh lệch (có thể âm) vào kỳ mở tiếp (`ky_bu_*`), tham chiếu batch + kỳ gốc. Seam lương đọc theo `ky_bu_*`.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `batch_id` | `Integer` FK→`san_xuat_batch.id` (SET NULL) | **IX** | yes | — | Batch gốc được bù trừ. |
| `phan_bo_id` | `Integer` FK→`san_xuat_phan_bo.id` (SET NULL) | — | yes | — | Phân bổ gốc (tuỳ chọn). |
| `employee_id` | `Integer` FK→`employees.id` (CASCADE) | **IX** | no | — | Người được bù trừ. |
| `department_id` | `Integer` FK→`departments.id` (SET NULL) | — | yes | — | Tổ ghi nhận. |
| `ky_goc_nam` | `Integer` | — | no | — | Năm kỳ gốc (đã khoá). |
| `ky_goc_thang` | `Integer` | — | no | — | Tháng kỳ gốc. |
| `ky_bu_nam` | `Integer` | **IX** | no | — | Năm kỳ bù (kỳ mở). |
...
### `san_xuat_phan_bo_loai_tru`

> 🔴 **BẢNG ĐÃ DROP 18/09/2026 (mg `0322`)** — xoá TRỌN tầng chia sản lượng cho từng người. Chủ xưởng: *"mẻ đó 3 người ghi sản lượng 3000 và thay kẽm số lượng 2, thì ghi nhận thế thôi đừng có chia bất cứ gì"* · *"tôi sẽ có cách chia khi sang kế toán sau"*. Mục dưới giữ để tra lịch sử.

**Purpose:** NGƯỜI bị LOẠI khỏi lương của MỘT batch (§7.3, Giai đoạn 4). Bảng MỚI (`create_all`), bảng TRẠNG THÁI (không `version`, gỡ = XOÁ dòng). Khi một người tham gia nhưng KHÔNG có phút chấm công hợp lệ (thiếu/quên chấm công) và không thể bổ sung, tổ trưởng xác nhận loại họ khỏi đợt trả lương của batch kèm LÝ DO — engine bỏ họ khỏi vòng chia trọng số (phần chia lại cho người còn lại) và cờ "thiếu chấm công" của họ tan để cho phép chốt. Vết ai-gì-lúc-nào nằm ở `AuditLog`.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `batch_id` | `Integer` FK→`san_xuat_batch.id` (CASCADE) | **IX**, **UQ**(batch_id, employee_id) | no | — | Batch bị loại người. |
| `employee_id` | `Integer` FK→`employees.id` (CASCADE) | **IX**, **UQ**(batch_id, employee_id) | no | — | Người bị loại khỏi lương batch. |
| `ly_do` | `String(500)` | — | no | — | Lý do loại (bắt buộc). |
| `created_by_id` | `Integer` FK→`users.id` | — | yes | — | Tổ trưởng thực hiện. |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |
...
**Purpose:** MỘT LẦN KIỂM một công đoạn (KCS theo lệnh, mg `0306`, `docs/design-kcs-theo-lenh.md`). `so_luong_nhan = so_luong_dat + so_luong_khong_dat` (service kiểm). Bản ghi chất lượng thuần: không đẻ `san_xuat_batch`, không trừ số, không đổi trạng thái công việc. Kiểm nhiều lần được; Σ đạt ở công đoạn `la_kcs_cuoi` (trần = Σ tốt tổ đã ghi) là số đề nghị nhập kho. Người kiểm = `created_by` (thành viên tổ `departments.is_kcs`).

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `cong_viec_id` | `Integer` FK→`san_xuat_cong_viec.id` (CASCADE) | IX | no | — | Công đoạn được kiểm (snapshot công việc, của bất kỳ tổ nào). |
| `nhom_id` | `Integer` FK→`san_xuat_nhom.id` (SET NULL) | IX | yes | — | Nhóm thành phẩm đang kiểm (định danh lô cho nhập kho §14.1). |
| `bat_dau` | `DateTime(timezone=True)` | — | no | — | Lúc ghi kết quả — **UTC THẬT**. |
| `ket_thuc` | `DateTime(timezone=True)` | — | no | — | Bằng `bat_dau` (kiểm là một lần ghi, không có cửa sổ). |
| `so_luong_nhan` | `Numeric(18,3)` | — | no | — | Số đã kiểm = đạt + lỗi. |
| `co_mau` | `Numeric(18,3)` | — | yes | — | Cỡ mẫu kiểm. |
| `so_luong_dat` | `Numeric(18,3)` | — | no | — | Số đạt (ở công đoạn cuối là nền nhập kho thành phẩm). |
| `so_luong_khong_dat` | `Numeric(18,3)` | — | no | `0` | Số lỗi (chi tiết ở `san_xuat_kcs_loi`). |
| `don_vi` | `String(24)` | — | no | — | Đơn vị. |
...
**Tất cả cột:** `id`, `cong_viec_id`, `nhom_id`, `bat_dau`, `ket_thuc`, `so_luong_nhan`, `co_mau`, `so_luong_dat`, `so_luong_khong_dat`, `don_vi`, `ket_luan`, `ghi_chu`, `checklist_json`, `version`, `created_by`, `created_at`, `updated_at`.

---

### `san_xuat_kcs_loi`

**Purpose:** phần LỖI của một lần kiểm (mg `0306`). Mô tả + số lượng + ≥1 ảnh. `to_chiu_id` = tổ của công đoạn bị kiểm, `cong_doan_ref_id` = chính công việc đó — server tự điền. Thông báo MỘT CHIỀU: tổ chịu bấm "Đã xem" (`phan_hoi_by_id` / `phan_hoi_luc`), không nhận / từ chối, không chặn gì. Lỗi có trước mg `0306` được đánh dấu đã xem lúc migration.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `kcs_batch_id` | `Integer` FK→`san_xuat_kcs_batch.id` (CASCADE) | IX | no | — | Lần kiểm phát hiện lỗi. |
| `mo_ta` | `String(500)` | — | yes | — | Mô tả lỗi (bắt buộc ở service khi ghi mới). |
| `to_chiu_id` | `Integer` FK→`departments.id` (SET NULL) | IX | yes | — | Tổ chịu = tổ của công đoạn bị kiểm. |
| `cong_doan_ref_id` | `Integer` FK→`san_xuat_cong_viec.id` (SET NULL) | IX | yes | — | Công đoạn bị kiểm. |
| `so_luong` | `Numeric(18,3)` | — | no | `0` | Số lượng dính lỗi. |
| `don_vi` | `String(24)` | — | yes | — | Đơn vị. |
| `phan_hoi_by_id` | `Integer` FK→`users.id` | — | yes | — | Người của tổ chịu bấm "Đã xem". |
| `phan_hoi_luc` | `DateTime(timezone=True)` | — | yes | — | Lúc bấm "Đã xem". NULL = tổ chưa xem. |
| `version` | `Integer` | — | no | `1` | Chống bấm trùng. |
| `created_by` | `Integer` FK→`users.id` | — | yes | — | Người ghi lỗi (KCS). |
| `created_at` | `DateTime(timezone=True)` | — | no | now (UTC) | |
| `updated_at` | `DateTime(timezone=True)` | — | no | now (UTC) | onupdate = now. |

**Tất cả cột:** `id`, `kcs_batch_id`, `mo_ta`, `to_chiu_id`, `cong_doan_ref_id`, `so_luong`, `don_vi`, `phan_hoi_by_id`, `phan_hoi_luc`, `version`, `created_by`, `created_at`, `updated_at`.

---

### `san_xuat_kcs_loi_anh`

**Purpose:** ẢNH bằng chứng của một lỗi KCS (§13.2, Giai đoạn 5). Bảng MỚI (`create_all`), CHỈ-THÊM (không `version`). Mỗi lỗi bắt buộc ≥1 ảnh (service kiểm). `file_url` soft ref (không ORM StoredFile) theo precedent `quote_attachments` / `ky_thuat_may_anh`; file phục vụ qua `/api/files` prefix `san-xuat`.

| Column | Type | Key | Null | Default | Meaning |
| --- | --- | --- | --- | --- | --- |
| `id` | `Integer` | **PK** | no | auto | Surrogate PK. |
| `loi_id` | `Integer` FK→`san_xuat_kcs_loi.id` (CASCADE) | IX | no | — | Lỗi KCS chứa ảnh. |
| `file_name` | `String(255)` | — | no | — | Tên file gốc. |
| `file_url` | `String(500)` | — | no | — | URL nội bộ (`/api/files/...`). |
| `file_type` | `String(100)` | — | yes | — | MIME type. |
| `uploaded_by` | `Integer` FK→`users.id` | — | yes | — | Người tải lên. |
...
**Purpose:** một HẠNG MỤC KIỂM của MỘT công đoạn — vd "Chồng màu đúng mẫu đã ký" của công đoạn In offset. Người khai đi ba tầng Giai đoạn → Công đoạn → hạng mục (`docs/design-kcs-theo-cong-doan.md`), nên hạng mục sinh ra đã nằm dưới một công đoạn; cùng câu chữ cho hai công đoạn thì khai hai dòng. GIAI ĐOẠN không lưu ở đây — nó là `cong_doan.nhom`. Bảng nối nhiều-nhiều `san_xuat_kcs_tieu_chi_cong_doan` đã GỠ ở mg `0285` (dữ liệu cũ tách thành nhiều dòng, mỗi công đoạn một dòng).
...
## Nhân sự & Lương — danh mục khoán · lịch sử ca
...
| `in_insurance_base` | `Boolean` → `BOOLEAN` | — | no | `false` | Có cộng vào GỐC ĐÓNG BẢO HIỂM không. Mặc định KHÔNG — gốc đóng BH là `luong_vi_tri` (chủ chốt 2026-07-20), phụ cấp không đụng vào. **DORMANT 07/09/2026** — engine không đọc (mức đóng BH = vị trí + trách nhiệm, chốt 12/08); API không nhận/trả nữa. Giữ cột. |
...
| `don_gia_ban` | `BigInteger` → `BIGINT` | — | yes | — | **GIÁ BÁN** một đơn vị — chỉ dòng NHẬP thành phẩm từ KCS: Σ thành tiền cụm bán ÷ SL cụm, lấy từ đơn (mg 0309). Chỉ để đọc (tồn kho, xuất giao hàng); KHÔNG vào sổ / NXT. Giá gốc vào sổ vẫn là `don_gia` (= 0 lúc nhập, kế toán kho sửa sau). NULL = đơn chưa có thành tiền hoặc không phải thành phẩm. |
...
- Primary key: `id`. Indexes: `request_id`, `material_id`, `ix_stock_request_lines_hang` on (`hang_loai`, `hang_id`) — migration `0301`.
...
- Primary key: `id`. Indexes: `voucher_id`, `request_line_id`, `material_id`, `lot_id`, `lo_goc_id`, `ix_stock_voucher_lines_hang` on (`hang_loai`, `hang_id`) — migration `0301`.
...
**Purpose:** một tài sản cố định, hoặc một LÔ công cụ dụng cụ cùng loại mua cùng lượt (12 tấm cao
su = 1 dòng, `so_luong` = 12). Phân biệt bằng `loai`, KHÔNG tách bảng — luật tính giống hệt nhau.

| Column                  | Type (SQLAlchemy → SQLite / Postgres)                 | Key                          | Null | Default        | Meaning                                                                                     |
| ----------------------- | ----------------------------------------------------- | ---------------------------- | ---- | -------------- | ------------------------------------------------------------------------------------------- |
| `id`                    | `Integer` → `INTEGER` / `SERIAL`                       | **PK**                       | no   | auto-increment | Surrogate primary key.                                                                        |
| `ma`                    | `String(32)` → `VARCHAR(32)`                           | **U**, **IX**                | no   | —              | Mã hệ sinh: `TS-0001` (TSCĐ) / `CC-0001` (CCDC).                                             |
| `ten`                   | `String(255)` → `VARCHAR(255)`                         | —                            | no   | —              | Tên tài sản.                                                                                  |
| `loai`                  | `String(8)` → `VARCHAR(8)`                             | **IX**                       | no   | `tscd`         | `tscd` \| `ccdc`.                                                                             |
| `so_luong`              | `Integer` → `INTEGER`                                  | —                            | no   | `1`            | Số cái trong lô (CCDC). TSCĐ luôn 1.                                                          |
| `don_gia`               | `BigInteger` → `BIGINT`                                | —                            | yes  | —              | Đơn giá một cái — CCDC nhập theo lô dùng ô này thay bảng chi phí.                             |
| `nguyen_gia`            | `BigInteger` → `BIGINT`                                | —                            | no   | `0`            | Tổng `tai_san_chi_phi`, hoặc `so_luong × don_gia`. `BigInteger` vì nguyên giá máy in tràn int32. |
| `so_thang`              | `Integer` → `INTEGER`                                  | —                            | no   | —              | Số tháng khấu hao/phân bổ kế toán chọn (gợi ý: máy in 7–15 năm, CCDC ≤ 3 năm).                |
| `ngay_su_dung`          | `Date` → `DATE`                                        | —                            | no   | —              | Ngày đưa vào sử dụng — mốc trích tháng đầu (prorate theo ngày).                               |
...
**Purpose:** một dòng cấu thành nguyên giá (giá mua, vận chuyển, lắp đặt chạy thử…). Giữ lại để
giải thích 3,3 tỷ đến từ đâu — cộng lại đúng bằng `tai_san.nguyen_gia`.

| Column       | Type (SQLAlchemy → SQLite / Postgres) | Key                       | Null | Default        | Meaning                        |
| ------------ | ------------------------------------- | ------------------------- | ---- | -------------- | ------------------------------ |
| `id`         | `Integer` → `INTEGER` / `SERIAL`      | **PK**                    | no   | auto-increment | Surrogate primary key.         |
| `tai_san_id` | `Integer` → `INTEGER`                 | FK→`tai_san.id`, **IX**   | no   | —              | Tài sản chủ. `ON DELETE CASCADE`. |
| `dien_giai`  | `String(255)` → `VARCHAR(255)`        | —                         | no   | —              | Nội dung khoản chi.            |
| `so_tien`    | `BigInteger` → `BIGINT`               | —                         | no   | `0`            | Số tiền khoản đó.              |
...
- Nhiều `tai_san_chi_phi` thuộc một `tai_san`.

**Tất cả cột:** `id`, `tai_san_id`, `dien_giai`, `so_tien`.
...
**Purpose:** Một NHÓM DÙNG CHUNG dữ liệu của khối Kinh doanh. 1 dòng = 1 nhóm có tên (vd "Cặp KD 1"). Hai người cùng nhóm thì phạm vi "Của tôi" của họ ở bốn màn `tinh_gia_thanh` · `bao_gia` · `don_hang_ban` · `khach_hang` được hiểu rộng ra thành "của tôi + của người cùng nhóm". Nhóm MỞ RỘNG DỮ LIỆU, KHÔNG nâng quyền. Bảng mới (migration `0333`).
...
- Một `nhom_dung_chung` có nhiều `nhom_dung_chung_thanh_vien` (CASCADE xoá).

**Tất cả cột:** `id`, `ten`, `created_by`, `created_at`.

---

### `nhom_dung_chung_thanh_vien`

**Purpose:** Một NGƯỜI trong một nhóm dùng chung. 1 dòng = (nhóm, người). Một người ở được nhiều nhóm — tập người dùng chung của ai đó là HỢP của mọi nhóm người đó thuộc. Bảng mới (migration `0333`).
...
- Nhiều `nhom_dung_chung_thanh_vien` thuộc một `nhom_dung_chung` và trỏ tới một `users`.

**Tất cả cột:** `id`, `nhom_id`, `user_id`, `added_by`, `added_at`.
