...
Dependency INLINE. MODULE quyền = "kho" (dùng chung với các danh mục kho sẵn có).
Chỉ khai báo kho (mã / tên / vị trí / ghi chú); vận hành nhập/xuất/tồn là bản sau.
"""
...
def get_service(db: Annotated[Session, Depends(get_db)]) -> KhoHangService:
    return KhoHangService(KhoHangRepository(db))
...
def get_item(kho_id: int, svc: Service, _: Annotated[User, Depends(require_permission(MODULE, "read"))]):
    try:
...
def update_item(kho_id: int, payload: KhoHangIn, svc: Service,
    ...
    try:
        return KhoHangRow.model_validate(svc.update(kho_id, payload.model_dump(exclude_unset=True)))
