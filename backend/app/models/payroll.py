...
Các bảng chính:
  ...
  - `employee_salaries`  — lương ẤN ĐỊNH của 1 NV, versioned theo `effective_from` (điều chỉnh
                            = thêm bản ghi mới, giữ lịch sử). Mức lương khai tay từng người.
...
Cấu hình lương — hai cấp `NV → tổ` (KHÔNG còn hệ thống bậc lương):
  - `employee_salaries`           — lương vị trí (= lương cơ bản = mức đóng BH) + trách nhiệm +
                                    3 khoản PHỤ CẤP KHAI TAY (ca · thâm niên · khác). Bậc tay
                                    nghề đã gỡ hẳn khỏi hệ thống (17/09/2026).
...
class EmployeeSalary(Base):
    ...
    # DORMANT: một số tổng khai tay của bản ghi cũ — engine chỉ dùng làm fallback khi chưa tách
    # 2 ô vị trí/trách nhiệm (`_resolve_monthly`).
    base_amount: Mapped[float | None] = mapped_column(_MONEY, nullable=True)
