...
class StockRequestLineIn(BaseModel):
    ...
    # Đơn giá NHẬP do người yêu cầu khai (chỉ yêu cầu NHẬP). Phiếu kế thừa; kho không sửa.
    don_gia: int | None = Field(default=None, ge=0)
    # Quy đổi đơn vị do người yêu cầu khai (1 don_vi_phu = he_so_quy_doi × dvt tồn).
...
class StockVoucherLineIn(BaseModel):
    request_line_id: int
    # Dòng yêu cầu là HÀNG MỚI (chưa có mã): kho CHỌN mã có sẵn → điền `material_id`; hoặc TẠO
...
class StockMaterialCreate(BaseModel):
    """Thêm nhanh mặt hàng ngay ở yêu cầu — tên + ĐVT (giá nhập ở bước phiếu).
