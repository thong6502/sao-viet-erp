...
- SEAM-13 (GỠ 2026-08-08, Đợt 5): Báo giá ← Tính giá. Cổng ``CostingResultPort`` +
  ``EstimateCostingAdapter`` đã xoá cùng cụm tính giá đời cũ (Estimate). Nguồn giá vốn giờ là
  Phiếu tính giá (``PhieuTinhGia``/``PhieuThanhPhan``): ``QuotationService._create_from_ptg``
  đọc thẳng ``gia_von_tp`` và KHÓA nó vào ``QuoteItem.total_cost_snapshot`` (copy-on-write,
  P0 §34) — không còn qua adapter, không còn FK sống tới bảng giá.
