...
class PricingEngine:
    ...
    def calculate_option(self, input_spec: dict, qty: int, at_date: date | None = None) -> tuple[list[EstimateCostLine], float, list[dict]]:
        ...
        if machine and machine_rate and machine.machine_type == "offset":
            ...
            if plate_rate:
                # Số bản kẽm: quy tắc bình bài (so_kem) nếu có; else số màu × số bộ kẽm (=số mặt) × form.
                plates_count = (so_kem_override if so_kem_override is not None
                                else int(round(colors * plate_set_factor * forms)))
