...
bằng import LAZY trong thân hàm. Lỗi nay dùng `XepLichValidationError` của `xep_lich_service`:
lớp `XepLich2Error` cũ đã đi cùng màn v2 (18/09/2026), mà router xưa nay vẫn map hai lớp ấy ra
cùng một mã lỗi nên hợp đồng HTTP không đổi.
"""
...
from ..xep_lich_service import XepLichValidationError
...
def tach(db: Session, *, dong_id: int, cac_phan: list[float],
    ...
    if goc is None:
        raise XepLichValidationError("Không tìm thấy dòng lịch.")
    if goc.is_locked:
        raise XepLichValidationError("Dòng đang khóa — mở khóa trước khi tách lần chạy.")
    if goc.phan_doan_tong > 1:
        raise XepLichValidationError("Dòng đã tách rồi — gộp lại trước khi chia kiểu khác.")
    if len(cac_phan) < 2:
        raise XepLichValidationError("Tách phải có ít nhất 2 phần.")
    ...
    if any(float(p) <= 0 for p in cac_phan):
        raise XepLichValidationError("Mỗi phần phải lớn hơn 0.")
    tong = round(
    ...
    if abs(sum(float(p) for p in cac_phan) - tong) > _EPS:
        raise XepLichValidationError(f"Tổng các phần phải bằng {tong:g}.")
...
def gop(db: Session, *, dong_id: int) -> XepLichCongDoan:
    ...
    if any(d.is_locked for d in cum):
        raise XepLichValidationError("Có phân đoạn đang khóa — mở khóa trước khi gộp.")
    goc = cum[0]
