"""Cầu MỘT CHIỀU từ bàn Xếp lịch sang bốn chỗ tiêu thụ lịch ngoài màn xếp lịch.
...
def moc_theo_buoc(db: Session, lsx_ids: list[int]) -> dict[int, tuple[datetime, datetime]]:
    ...
    from .service import XepLichLenhService

    svc = XepLichLenhService(db, XepLichLenhRepository(db))
