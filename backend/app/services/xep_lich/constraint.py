"""Luật THỜI GIAN thuần của xếp lịch — HÀM LÁ, không chạm DB, không chạm engine cũ.

Viết cho bàn theo công đoạn (`xep_lich_2`, gỡ 18/09/2026), giữ lại nguyên vẹn vì đây là tầng LUẬT,
không phải tầng màn hình: `services/san_xuat/release.py` và `xep_lich_van_de_service` vẫn gọi vào.
...
def tron_phut(moc: datetime | None, *, len_tren: bool = False) -> datetime | None:
    """Cắt giây/micro giây — mọi mốc xếp lịch ĐƯA RA hay GHI XUỐNG đều phải TRÒN PHÚT.
