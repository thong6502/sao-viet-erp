        ...
        {canApprove && row.status === "pending_approval" && (
          <>
            <Button
              type="button"
              variant="primary"
              onClick={() => setVoucherMode({ purchase: row, combined: true })}
            >
              Duyệt & lập chứng từ
            </Button>
            <Button
        ...
        {canApprove &&
          ...
          row.available_amount > 0 && (
            <Button
              type="button"
              variant="primary"
              onClick={() => setVoucherMode({ purchase: row, combined: false })}
            >
              Lập Phiếu chi/UNC
            </Button>
          )}
    ...
    <main className="md-page">
      <header className="md-page__head">
        ...
        <p className="md-page__sub">
          Duyệt PMH Thu mua gửi đến và lập Phiếu chi hoặc Ủy nhiệm chi từ cùng
          một chứng từ nguồn.
        </p>
      </header>
          ...
          <table className="md-page__table">
            ...
            <tbody>
              ...
              {!loading && rows.length === 0 && (
                <tr>
                  <td colSpan={6}>Không có phiếu mua hàng phù hợp.</td>
                </tr>
              )}
              {!loading &&
                rows.map((row) => {
        ...
        <aside className="purchase__detail">
          {!selected ? (
            <div className="purchase__empty-detail">
              Chọn một PMH để xem chi tiết.
            </div>
          ) : (
            <>
              <div className="purchase__detail-head">
                <div>
                  <p className="eyebrow">Chi tiết PMH</p>
                  <h2>{selected.code}</h2>
                </div>
                <span
