...
export function AccountingBankAccountsPage() {
  ...
  function cleanForm(): BankAccountInput {
    return {
  ...
  async function save(event: FormEvent) {
    ...
    try {
      if (tab === "company") {
        ...
        const supplierPayload: SupplierBankAccountInput = { ...payload, supplier_id: formSupplierId! };
  ...
  return (
    <main className="md-page">
      ...
      <section className="card md-page__tablewrap">
        <table className="md-page__table"><thead><tr>{tab === "supplier" && <th>Nhà cung cấp</th>}<th>Chủ tài khoản</th><th>Số tài khoản</th><th>Ngân hàng</th><th>Chi nhánh</th><th>Loại tiền</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>
          {loading && <tr><td colSpan={tab === "supplier" ? 8 : 7}>Đang tải...</td></tr>}
          {!loading && rows.length === 0 && <tr><td colSpan={tab === "supplier" ? 8 : 7}>Chưa có tài khoản ngân hàng.</td></tr>}
          {!loading && rows.map((row) => <tr key={`${tab}-${row.id}`}>{tab === "supplier" && <td>{"supplier_name" in row ? row.supplier_name : "—"}</td>}<td><strong>{row.account_holder}</strong>{row.is_default && <small className="acct-default-tag">Mặc định</small>}</td><td>{row.account_number}</td><td>{row.bank_name}</td><td>{row.bank_branch}</td><td>{row.currency}</td><td><span className={`acct-voucher-status acct-voucher-status--${row.is_active ? "paid" : "cancelled"}`}>{row.is_active ? "Hoạt động" : "Ngừng dùng"}</span></td><td><div className="acct-actions acct-actions--compact">{canManage && <><Button variant="ghost" onClick={() => openEdit(row)}>Sửa</Button><Button variant="ghost" disabled={busy} onClick={() => toggle(row)}>{row.is_active ? "Ngừng dùng" : "Bật lại"}</Button></>}</div></td></tr>)}
        </tbody></table>
      </section>
      ...
      {modalOpen && <div className="acct-modal" role="dialog" aria-modal="true"><form className="acct-modal__box" onSubmit={save}><header className="acct-modal__head"><h2>{editing ? "Sửa tài khoản" : "Thêm tài khoản"}</h2><button type="button" className="acct-modal__x" onClick={closeModal}>×</button></header><div className="acct-modal__body">
        ...
        <div className="acct-checks"><label><input type="checkbox" checked={form.is_default} onChange={(event) => setForm((current) => ({ ...current, is_default: event.target.checked }))} /> Tài khoản mặc định</label><label><input type="checkbox" checked={form.is_active} onChange={(event) => setForm((current) => ({ ...current, is_active: event.target.checked }))} /> Đang hoạt động</label></div>
