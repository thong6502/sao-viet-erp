...
import type {
  XlChiTiet as XlChiTietData,
  XlCongDoan as XlCongDoanData,
  XlGoiPhatHanh,
  XlSoSanh,
} from "../api/client";
import { gio, gioChu, moc, ngayNgan, quangDongHo, thuNgay, treHan } from "./xlShared";
import type { XlPhanTachNghi } from "./xlShared";
...
function getCongDoanTheme(ten: string): { icon: JSX.Element; colorClass: string } {
  ...
  if (t.includes("ctp") || t.includes("bản") || t.includes("kẽm")) {
    return { icon: <Layers size={13} />, colorClass: "xl-c-icon--ctp" };
  }
  if (t.includes("cắt") || t.includes("xả")) {
    return { icon: <Scissors size={13} />, colorClass: "xl-c-icon--cat" };
  }
  if (t.includes("in")) {
    return { icon: <Printer size={13} />, colorClass: "xl-c-icon--in" };
  }
  if (t.includes("cán") || t.includes("màng") || t.includes("phủ") || t.includes("uv") || t.includes("ép kim")) {
    return { icon: <Sparkles size={13} />, colorClass: "xl-c-icon--can" };
  }
  if (t.includes("bế") || t.includes("dập")) {
    return { icon: <Box size={13} />, colorClass: "xl-c-icon--be" };
  }
  if (t.includes("dán") || t.includes("gấp") || t.includes("đóng cuốn")) {
    return { icon: <FoldVertical size={13} />, colorClass: "xl-c-icon--dan" };
  }
  if (t.includes("kcs") || t.includes("đóng gói") || t.includes("giao")) {
    return { icon: <PackageCheck size={13} />, colorClass: "xl-c-icon--kcs" };
  }
  return { icon: <Box size={13} />, colorClass: "xl-c-icon--default" };
}
...
function nhanMay(c: XlCongDoanData): JSX.Element {
  ...
  return (
    <>
      ...
      {c.may_ke_hoach_ten && (
        <span className="xl-c-may-kh" title={`Kế hoạch ban đầu: ${c.may_ke_hoach_ten} — xưởng đã đổi máy`}>
...
function nhanSl(c: XlCongDoanData): string {
      ...
      <div className="xl-modal xl-modal--compact" role="dialog" aria-modal="true">
        {dangTai && !ct ? (
          ...
          <>
            ...
            <header className="xl-modal__dau xl-modal__dau--compact">
              <div className="xl-modal__dau-trai">
                <div className="xl-modal__ma-cum">
                  <span className="xl-modal__ma">{ct.ma}</span>
                  <button
                    type="button"
                    onClick={handleCopy}
                    className="xl-copy-btn"
                    title="Sao chép mã lệnh"
                  >
                    {copied ? <Check size={12} color="var(--moss-deep)" /> : <Copy size={12} />}
                    <span>{copied ? "Đã chép" : "Mã LSX"}</span>
                  </button>
                  {ct.is_rush && <span className="xl-gap">GẤP</span>}
                  <span className={`xl-tt xl-tt--pulse xl-tt--${ct.trang_thai}`}>
                    {TRANG_THAI_NHAN[ct.trang_thai] ?? ct.trang_thai}
                  </span>
                </div>
                <div className="xl-modal__sub-line">
                  <span className="xl-modal__ten-sp">{ct.ten || "—"}</span>
                  <span className="xl-modal__sub-sep">·</span>
                  <span className="xl-modal__sub-kh">{ct.customer_name ?? "Khách lẻ"}</span>
                  {ct.customer_po_no && <span className="xl-modal__sub-po">PO: {ct.customer_po_no}</span>}
                </div>
              </div>
              <button type="button" className="xl-modal__dong" onClick={onDong} aria-label="Đóng popup">
                <X size={16} />
              </button>
            </header>

            {/* Thân Modal 2 Cột Studio */}
            <div className="xl-modal__than">
              ...
              <div className="xl-modal__col-trai">
                ...
                <div className={`xl-modal__card${tre !== null && tre > 0 ? " xl-modal__card--tre" : ""}`}>
                  ...
                  <div className="xl-modal__input-wrap">
                    <input
                      type="datetime-local"
                      className="xl-modal__input"
                      value={gioInput}
                  ...
                  {daXep && (
                    <>
                      ...
                      <div className="xl-timeline-strip">
                        <div className="xl-timeline-main">
                          <div className="xl-timeline-node">
                            ...
                            <span className="xl-timeline-val">{gioThu(ct.bat_dau_at)}</span>
                          </div>
                          ...
                          <div className="xl-timeline-node xl-timeline-node--end">
                            <span className="xl-timeline-label">DỰ KIẾN XONG</span>
                            <span className="xl-timeline-val">{gioThu(ct.ket_thuc)}</span>
                          </div>
                        </div>

                        <div className="xl-timeline-footer">
                          <span className="xl-timeline-badge" title="Tổng thời gian thực hiện">
                            <Clock size={11} />
                            Tổng khoảng tính: <strong>{gioChu(((moc(ct.ket_thuc) ?? 0) - (moc(ct.bat_dau_at) ?? 0)) / 60_000)}</strong>
                          </span>
                          {daChayDo && (
                            <span className="xl-timeline-subnote">
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="xl-kpi-block">
                            <div className="xl-kpi-split-bar" title={`Tỉ lệ thời gian: ${pctChay}% giờ làm, ${pctNghi}% nghỉ & ngoài ca`}>
                              <div className="xl-kpi-split-fill xl-kpi-split-fill--chay" style={{ width: `${pctChay}%` }} />
                              <div className="xl-kpi-split-fill xl-kpi-split-fill--nghi" style={{ width: `${pctNghi}%` }} />
                            </div>

                            <div className="xl-modal__stats">
                              <div
                                className="xl-modal__stat xl-modal__stat--chay"
                                title="Cộng giờ làm của các bước: bước máy gồm cả chuẩn bị máy, bước tổ tính theo năng suất người"
                              >
                                <div className="xl-modal__stat-head">
                                  <span className="xl-modal__stat-k"><PlayCircle size={11} /> Giờ làm</span>
                                  <span className="xl-modal__stat-pct">{pctChay}%</span>
                                </div>
                                <span className="xl-modal__stat-v">{gioChu(chayM)}</span>
                              </div>

                              <div className="xl-modal__stat xl-modal__stat--nghi">
                                <div className="xl-modal__stat-head">
                                  <span className="xl-modal__stat-k"><PauseCircle size={11} /> Nghỉ &amp; ngoài ca</span>
                                  <span className="xl-modal__stat-pct">{pctNghi}%</span>
                                </div>
                                <span className="xl-modal__stat-v">{gioChu(nghiM)}</span>
                              </div>
                            </div>
                          </div>
                        );
                      ...
                      {phanTach && (
                        <div className="xl-shift-card">
                          <div className="xl-shift-head">
                            <span className="xl-shift-title">Chi tiết ca &amp; thời gian nghỉ</span>
                          </div>

                          {/* Ca sản xuất */}
                          {phanTach.cac_ca?.length ? (
                            <div className="xl-shift-section">
                              <span className="xl-shift-sec-label">Ca sản xuất:</span>
                              <div className="xl-shift-chips">
                                {phanTach.cac_ca.map((c) => (
                                  <span key={`${c.ten}-${c.tu}`} className="xl-chip xl-chip--ca">
                                    <strong>{c.ten}</strong> {c.tu}–{c.den}
                                    {c.nghi_tu && <span className="xl-chip-sub"> (nghỉ {c.nghi_tu}–{c.nghi_den})</span>}
                                  </span>
                                ))}
                              </div>
                            </div>
                            ...
                            <div className="xl-shift-section">
                              ...
                              <div className="xl-shift-chips">
                                {phanTach.ca_san_xuat.map((k, idx) => (
                                  <span key={idx} className="xl-chip xl-chip--ca">
                                    {k.tu}–{k.den}
                                  </span>
                                ))}
                              </div>
                          ...
                          {phanTach.nghi_giua_ca_phut > 0 && (
                            <div className="xl-shift-row">
                              <div className="xl-shift-meta">
                                <span className="xl-shift-k">Nghỉ giữa ca</span>
                                <b className="xl-shift-v">{gioChu(phanTach.nghi_giua_ca_phut)}</b>
                              </div>
                              <div className="xl-shift-chips">
                                {phanTach.nghi_giua_ca.map((k, i) => (
                                  <span key={i} className="xl-chip xl-chip--nghi">
                                    {k.tu}–{k.den}
                                    {k.so_lan > 1 && <span className="xl-chip-tag">{k.so_lan} lần</span>}
                                  </span>
                                ))}
                              </div>
                            </div>
                          ...
                          {phanTach.ngoai_ca_phut > 0 && (
                            <div className="xl-shift-row">
                              <div className="xl-shift-meta">
                                <span className="xl-shift-k">Ngoài ca</span>
                                <b className="xl-shift-v">{gioChu(phanTach.ngoai_ca_phut)}</b>
                              </div>
                              <div className="xl-shift-chips">
                                {phanTach.ngoai_ca.map((k, i) => (
                                  <span key={i} className="xl-chip xl-chip--ngoai">
                                    {k.tu}–{k.den}
                                    {k.so_lan > 1 && <span className="xl-chip-tag">{k.so_lan} lần</span>}
                                  </span>
                                ))}
                          ...
                          {phanTach.ngay_nghi_phut > 0 && (
                            <div className="xl-shift-row">
                              <div className="xl-shift-meta">
                                <span className="xl-shift-k">Ngày nghỉ</span>
                                <b className="xl-shift-v">{gioChu(phanTach.ngay_nghi_phut)}</b>
                              </div>
                              <div className="xl-shift-chips">
                                {phanTach.ngay_nghi.map((n, i) => (
                                  <span key={i} className="xl-chip xl-chip--ngay">
                                    {thuNgay(n.ngay)}
                                    {n.ten && ` (${n.ten})`}
                          ...
                          {phanTach.gia_cong_ngoai_phut > 0 && (
                            <div className="xl-shift-row">
                              <div className="xl-shift-meta">
                                <span className="xl-shift-k">Gia công ngoài</span>
                                <b className="xl-shift-v">{gioChu(phanTach.gia_cong_ngoai_phut)}</b>
                              </div>
                              <span className="xl-shift-subtxt">bên gia công giữ hàng</span>
                            </div>
                          )}
                        </div>
                      )}
                  ...
                  {ct.co_thuc_te && (
                    <div className={`xl-modal__ketqua xl-modal__ketqua--tt xl-modal__ketqua--${lechThucTe?.cls ?? "khit"}`}>
                      ...
                      <div className="xl-modal__ketqua-body">
                        ...
                        {lechThucTe && (
                          <span
                            className={`xl-modal__ketqua-badge xl-modal__ketqua-badge--${lechThucTe.cls}`}
                ...
                <div className="xl-deadline-grid">
                  <div
                    className={`xl-deadline-card xl-deadline-card--sx xl-deadline-card--${demNguocSx.loai}`}
                    title="Mốc kinh doanh yêu cầu xưởng phải xong — KHÔNG phải ngày xong theo lịch đã xếp"
                  >
                    <div className="xl-deadline-head">
                      <Target size={13} className="xl-deadline-icon" />
                      <span className="xl-deadline-label">Mục tiêu hoàn thành SX</span>
                    </div>
                    <div className="xl-deadline-body">
                      <span className="xl-deadline-val">{ngayNgan(ct.han_hoan_thanh_sx)}</span>
                      {ct.han_hoan_thanh_sx && (
                        <span
                          className={`xl-deadline-badge xl-deadline-badge--${demNguocSx.loai}`}
                  ...
                  <div className={`xl-deadline-card xl-deadline-card--giao xl-deadline-card--${demNguocGiao.loai}`}>
                    <div className="xl-deadline-head">
                      <Truck size={13} className="xl-deadline-icon" />
                      <span className="xl-deadline-label">Hạn Giao Khách</span>
                    </div>
                    <div className="xl-deadline-body">
                      <span className="xl-deadline-val">{ngayNgan(ct.han_giao_khach)}</span>
                      {ct.han_giao_khach && (
                        <span className={`xl-deadline-badge xl-deadline-badge--${demNguocGiao.loai}`}>
                          {demNguocGiao.nhan}
                        </span>
                      )}
                    </div>
                ...
                <div className="xl-modal__card">
                  <div className="xl-modal__card-tieu">
                    <Tag size={13} /> Quy cách & Vật tư
                  </div>
                  <div className="xl-spec-table">
                    <div className="xl-spec-row">
                      <span className="xl-spec-k">Sản lượng đặt</span>
                      <span className="xl-spec-v">{ct.so_luong_dat.toLocaleString("vi-VN")} {ct.don_vi_tinh ?? "sp"}</span>
                    </div>
                    <div className="xl-spec-row">
                      <span className="xl-spec-k">Tờ kế hoạch</span>
                      <span className="xl-spec-v">
                        {ct.so_to_ke_hoach.toLocaleString("vi-VN")} tờ
                        {ct.so_con > 1 ? <span className="xl-spec-sub"> ({ct.so_con} con)</span> : ""}
                      </span>
                    </div>
                    {ct.giay && (
                      <div className="xl-spec-row">
                        <span className="xl-spec-k">Loại giấy</span>
                        <span className="xl-spec-v">
                          <span className="xl-paper-pill">{ct.giay}</span>
                        </span>
                      </div>
                    )}
                    {ct.kho_in && (
                      <div className="xl-spec-row">
                        <span className="xl-spec-k">Khổ & Màu</span>
                        <span className="xl-spec-v">
                          <span className="xl-spec-pill">{ct.kho_in}{ct.so_mau ? ` · ${ct.so_mau}m` : ""}</span>
                        </span>
                      </div>
                    )}
                    {ct.sale_name && (
                      <div className="xl-spec-row">
                        <span className="xl-spec-k"><UserCheck size={11} style={{ display: "inline", marginRight: 4 }} />Kinh doanh</span>
                        <span className="xl-spec-v">{ct.sale_name}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {phienBans.length > 0 && (
                  <div className="xl-modal__card xl-ls">
                    <button
                      type="button"
                      className="xl-ls-dau"
                    ...
                    >
                      <span className="xl-modal__card-tieu" style={{ margin: 0 }}>
                        <History size={13} /> Lịch sử phát hành
                      </span>
                      <span className="xl-ls-dem">{phienBans.length} phiên bản</span>
                    </button>

                    {moLichSu && (
                      <>
                        {/* Dòng hướng dẫn chọn phiên bản */}
                        <div className={`xl-ls-hint${chon.length > 0 ? " xl-ls-hint--active" : ""}`}>
                          {chon.length === 0 ? (
                            <span>💡 Bấm chọn 2 phiên bản để so sánh mốc lịch &amp; máy chạy</span>
                          ) : chon.length === 1 ? (
                            <span className="xl-ls-hint-row">
                              <span>💡 Đã chọn <strong>v{chon[0]}</strong> · Bấm thêm 1 bản nữa để so sánh</span>
                              <button type="button" className="xl-ls-bo-link" onClick={() => setChon([])}>
                                hủy
                              </button>
                            </span>
                          ) : (
                          ...
                          {phienBans.map((p) => {
                            ...
                            return (
                              <button
                                key={p.so}
                                type="button"
                                className={`xl-ls-item${dangChon ? " xl-ls-item--chon" : ""}`}
                                onClick={() => chonPhienBan(p.so)}
                                title="Bấm chọn 2 phiên bản để so lịch từng bước"
                              >
                                <span className="xl-ls-v-pill">v{p.so}</span>
                                <span className={`xl-ls-loai-badge xl-ls-loai-badge--${p.loai}`}>
                                  {p.loai === "cap_nhat" ? "cập nhật" : "phát hành"}
                                </span>
                                <span className="xl-ls-ly-do" title={p.ly_do || "Không có ghi chú"}>
                                  {p.ly_do || "—"}
                                </span>
                                <span className="xl-ls-luc">{gio(p.luc)}</span>
                              </button>
                            );
                          })}
                        </div>
                        ...
                        {/* Thẻ So Sánh Độc Lập */}
                        {capA != null && capB != null && (
                          <div className="xl-ls-compare-card">
                            <div className="xl-ls-compare-head">
                              <span className="xl-ls-compare-title">
                                <ArrowRightLeft size={12} /> So sánh v{capA} ➔ v{capB}
                              </span>
                              <button type="button" className="xl-ls-compare-close" onClick={() => setChon([])} title="Bỏ chọn">
                                <X size={12} /> Bỏ chọn
                              </button>
                            </div>

                            {loiSoSanh && <div className="xl-ls-loi">{loiSoSanh}</div>}
                            {!soSanh && !loiSoSanh && <div className="xl-ls-trong">Đang đối chiếu...</div>}
                            {soSanh && (
                              soSanh.dong.some((d) => d.doi_gio || d.doi_may) ? (
                                <div className="xl-ls-diff-list">
                                  {soSanh.dong.filter((d) => d.doi_gio || d.doi_may).map((d) => (
                                    <div key={d.cong_viec_id} className="xl-ls-diff-row">
                                      <span className="xl-ls-diff-step">{d.ten}</span>
                                      <div className="xl-ls-diff-details">
                                        {d.doi_gio && (
                                          <div className="xl-ls-diff-time">
                                            <span className="xl-ls-diff-old">{gio(d.a.bat_dau)}</span>
                                            <span className="xl-ls-diff-arrow">➔</span>
                                            <span className="xl-ls-diff-new">{gio(d.b.bat_dau)}</span>
                                          </div>
                                        )}
                                        {d.doi_may && (
                                          <div className="xl-ls-diff-machine">
                                            <span className="xl-ls-diff-old-m">máy {d.a.may_ten ?? "—"}</span>
                                            <span className="xl-ls-diff-arrow">➔</span>
                                            <span className="xl-ls-diff-new-m">máy {d.b.may_ten ?? "—"}</span>
                                          </div>
                                        )}
                                      </div>
                                    </div>
              ...
              <div className="xl-modal__col-phai">
                <div className="xl-pipe-head">
                  <div className="xl-pipe-title-wrap">
                    <span className="xl-modal__card-tieu" style={{ margin: 0 }}>
                      <Layers size={13} color="var(--rust)" /> Quy trình sản xuất
                    </span>
                  </div>
                  <span
                    className="xl-pipe-count"
                ...
                <div className="xl-pipe-card-container">
                  <div className="xl-pipe-list">
                    {ct.cong_doans.map((c, idx) => {
                      ...
                      return (
                        <div key={c.id} className={`xl-pipe-step${coThoiGian ? " xl-pipe-step--active" : ""}`}>
                          {/* Circle Node trên đường pipeline */}
                          <div className={`xl-pipe-node ${colorClass}`}>
                            <span className="xl-pipe-node-num">{idx + 1}</span>
                          </div>

                          {/* Thẻ nội dung công đoạn */}
                          <div className="xl-pipe-content">
                            <div className={`xl-c-icon ${colorClass}`}>{icon}</div>
                            <div className="xl-pipe-info">
                              <span className="xl-pipe-name">
                                {c.ten}
                                {c.song_song && (
                                  <span
                              ...
                              {coMoc && (
                                <span className="xl-pipe-moc">
                                  <span className="xl-pipe-moc-hang">
                                    <span className="xl-pipe-moc-k">KH</span>
                                    <span className="xl-pipe-moc-v">
                                      {gio(c.ke_hoach_bat_dau)} → {gio(c.ke_hoach_ket_thuc)}
                                    </span>
                                  </span>
                                  <span className="xl-pipe-moc-hang xl-pipe-moc-hang--tt">
                                    <span className="xl-pipe-moc-k">TT</span>
                                    <span className="xl-pipe-moc-v">
                                      {c.thuc_bat_dau
                                    ...
                                    {lech && (
                                      <span className={`xl-pipe-lech xl-pipe-lech--${lech.cls}`}>
                                        {lech.chu}
                                      </span>
                                    )}
                                  </span>
                            ...
                            <div className="xl-pipe-meta">
                              {c.so_nguoi_chuan > 0 && (
                                <span className="xl-c-nguoi" title="Số người tiêu chuẩn của bước">
                                  {c.so_nguoi_chuan} người
                                </span>
                              )}
                              <span
                                className={`xl-pipe-dur${
                                  coThoiGian
                                    ? " xl-pipe-dur--active"
                                    : c.canh_bao ? " xl-pipe-dur--thieu" : " xl-pipe-dur--wait"
                                }`}
            ...
            <footer className="xl-modal__chan xl-modal__chan--compact">
              <button type="button" className="xl-nut xl-nut--phu" onClick={onDong}>
                Đóng
              </button>
              <div className="xl-modal__actions">
                {duyetDuoc && daPhatHanh && khoaThuHoi && (
                  <span className="xl-modal__chan-nhac">
                    <AlertCircle size={12} />
                ...
                {duyetDuoc && daPhatHanh && (
                  <button
                    type="button"
                    className="xl-nut xl-nut--thu-hoi"
