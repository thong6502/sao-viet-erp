...
import type { XlDong } from "../api/client";
import {
...
export function XlGantt({
  ...
  return (
    <div className="xl-gantt">
      {/* Header Ngày Lưới Giàu Thông Tin — Adaptive theo N Ngày */}
      <div className="xl-gantt__head" style={{ paddingLeft: nhanW }}>
        {ngays.map((d) => {
          const n = nhanNgay(d);
          ...
          if (nac === 30) {
            return (
              <div
                key={d}
                className={`xl-gantt__ngay xl-gantt__ngay--v30${nghi.has(d) ? " xl-gantt__ngay--nghi" : ""}${n.homNay ? " xl-gantt__ngay--nay" : ""}`}
                style={{ width: ngayW }}
                title={`${n.thu} ${n.so}${hasLoad ? ` · ${st.count} lệnh` : ""}`}
              >
                <span className="xl-gantt__so-v30">{ngaySo}</span>
                <span className="xl-gantt__thu-v30">{n.thu}</span>
                {hasLoad && <span className="xl-day-dot" />}
              </div>
            );
          }
          ...
          if (nac === 14) {
            return (
              <div
                key={d}
                className={`xl-gantt__ngay xl-gantt__ngay--v14${nghi.has(d) ? " xl-gantt__ngay--nghi" : ""}${n.homNay ? " xl-gantt__ngay--nay" : ""}`}
                style={{ width: ngayW }}
              >
                <div className="xl-gantt__ngay-dong1">
                  <span className="xl-gantt__thu">{n.thu}</span>
                  <span className="xl-gantt__so">{n.so}</span>
                </div>
                {hasLoad && (
                  <span className="xl-day-badge xl-day-badge--sm" title={`${st.count} lệnh đang chạy trong ngày`}>
                    {st.count}L
                  </span>
                )}
              </div>
          ...
          return (
            <div
              key={d}
              className={`xl-gantt__ngay xl-gantt__ngay--v7${nghi.has(d) ? " xl-gantt__ngay--nghi" : ""}${n.homNay ? " xl-gantt__ngay--nay" : ""}`}
              style={{ width: ngayW }}
            >
              <div className="xl-gantt__ngay-dong1">
                <span className="xl-gantt__thu">{n.thu}</span>
              ...
              {hasLoad && (
                <span className="xl-day-badge" title={`${st.count} lệnh đang chạy trong ngày`}>
                  {st.count} lệnh
                </span>
              )}
            </div>
      ...
      <div
        className={`xl-gantt__body${keoTuHangCho ? " xl-gantt__body--nhan" : ""}`}
        ref={luoiRef}
        onDragOver={(e) => suaDuoc && keoTuHangCho && e.preventDefault()}
        onDrop={thaVaoLuoi}
      >
        {nowX !== null && (
          <div className="xl-now-line" style={{ left: nhanW + nowX }}>
            <span className="xl-now-dot" />
            <span className="xl-now-pill">HÔM NAY</span>
          </div>
        )}

        {dong.length === 0 && (
          <div className="xl-gantt__empty-container">
            {/* Hàng lưới rỗng tạo background chuẩn cho khung Gantt */}
            {Array.from({ length: 6 }).map((_, idx) => (
              <div key={idx} className="xl-hang xl-hang--trong" style={{ height: dongH }}>
                <div className="xl-hang__nhan" style={{ width: nhanW }} />
                <div className="xl-hang__luoi" style={{ width: rongLuoi }}>
                  {ngays.map((n, i) => (
                    <span
                      key={n}
                      className={`xl-cot${nghi.has(n) ? " xl-cot--nghi" : ""}`}
            ...
            <div className="xl-trong-box-wrap" style={{ paddingLeft: nhanW }}>
              <div className="xl-trong-box">
                <div className="xl-trong-icon-wrap">
                ...
                <div className={`xl-trong-hint${keoTuHangCho ? " xl-trong-hint--active" : ""}`}>
                  {keoTuHangCho ? (
                    <>
                      <Sparkles size={14} />
        ...
        {dong.map((d) => {
          ...
          return (
            <div
              key={d.lsx_id}
              className={`xl-hang${chonId === d.lsx_id ? " xl-hang--chon" : ""}`}
              style={{ height: dongH }}
              onClick={() => { if (!vuaKeoRef.current) onChon(d.lsx_id); }}
            >
              {/* Cột nhãn trái (260px màn rộng, co còn 184px khi màn hẹp — xem `khungLuoi`) */}
              <div className="xl-hang__nhan" style={{ width: nhanW }}>
                <div className="xl-hang__ma">
                  {d.is_rush && <span className="xl-gap" title="Lệnh gấp">GẤP</span>}
                  <span className="xl-hang__ma-text">{d.ma}</span>
                  <span className={`xl-tt xl-tt--${d.trang_thai}`}>
                ...
                {d.ten && (
                  <div className="xl-hang__ten-sp" title={d.ten}>
                    {d.ten}
                  </div>
                )}

                <div className="xl-hang__dong-khach">
                  <div className="xl-hang__phu" title={`${d.customer_name ?? "Khách lẻ"}${d.may_ten ? ` · ${d.may_ten}` : ""}`}>
                    <span className="xl-hang__khach">{d.customer_name ?? "Khách lẻ"}</span>
                    {d.may_ten && <span className="xl-hang__may"> · {d.may_ten}</span>}
                  </div>
                  {tre !== null && tre > 0 && (
                    <span className="xl-hang__tre" title={`Dự kiến xong sau hạn SX ${tre} ngày`}>Trễ hạn</span>
                  )}
                </div>
                ...
                {(() => {
                  ...
                  return <div className="xl-hang__sl" title={sl}>{sl}</div>;
                })()}
              </div>
              ...
              <div className="xl-hang__luoi" style={{ width: rongLuoi }}>
                ...
                {(() => {
                  ...
                  const vach = (ngay: string | null, loai: "sx" | "kh", chu: string, title: string) => {
                    ...
                    return (
                      <div
                        key={loai}
                        className={`xl-han xl-han--${loai}${lat ? " xl-han--lat" : ""}`}
                        style={{ left: hx }}
                        title={title}
                      >
                        <span className="xl-han-tag">
                          {cungNgay ? (
                            <>
                              <Target size={10} className="xl-han-svg" />
                              <Truck size={10} className="xl-han-svg" />
                            </>
                          ) : loai === "kh" ? (
                            <Truck size={10} className="xl-han-svg" />
                          ) : (
                          ...
                          <span className="xl-han-text">{chu}</span>
                        </span>
                      </div>
                    );
                  };
                ...
                {daVaoViec && (
                  <div
                    className="xl-thanh xl-thanh--cho"
                ...
                {bao && (
                  ...
                  >
                    {bao.tranTrai && <span className="xl-thanh__mui xl-thanh__mui--trai" />}
                    ...
                    <span className="xl-thanh__ten">
                      <strong>{d.ma}</strong>
                      {d.ten ? ` · ${d.ten}` : ""}
                      {d.so_luong_dat ? ` (${d.so_luong_dat.toLocaleString("vi-VN")} ${d.don_vi_tinh ?? "sp"})` : ""}
                    </span>

                    {bao.tranPhai && <span className="xl-thanh__mui xl-thanh__mui--phai" />}
                  </div>
                )}
                ...
                {bao && !bao.tranPhai && !dangKeo && (() => {
                  ...
                  return (
                    <span
                      className={`xl-xong${muon ? " xl-xong--tre" : ""}${lat ? " xl-xong--lat" : ""}`}
