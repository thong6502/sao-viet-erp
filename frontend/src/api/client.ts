...
export interface OrderDetail extends OrderRow {
  ...
  deposits: OrderDepositOut[];         // legacy — rỗng ở production
  receipts: OrderReceiptOut[];         // phiếu thu 01-TT của đơn (nguồn cọc)
...
export interface OrderUpdateInput {
  ...
  /** % cọc phải thu (0–100); chỉ Kế toán (record_deposit) đặt được, backend chặn Sale. */
  deposit_pct?: number | null;
  customer_po_no?: string | null;
  delivery_committed_date?: string | null;
  delivery_address?: string | null;
...
export const api = {
  ...
  orders: {
    ...
    // Phiếu thu 01-TT cho cọc (production) — Kế toán ghi
    createReceipt(token: string, id: number, input: OrderReceiptInput): Promise<OrderDetail> {
      return authed<OrderDetail>(`/api/orders/${id}/receipts`, token, { method: "POST", body: JSON.stringify(input) });
    },
    cancelReceipt(token: string, id: number, receiptId: number, reason: string): Promise<OrderDetail> {
      return authed<OrderDetail>(`/api/orders/${id}/receipts/${receiptId}/cancel`, token, { method: "POST", body: JSON.stringify({ reason }) });
    },
    uploadReceiptProof(token: string, id: number, receiptId: number, file: File): Promise<OrderDetail> {
      const form = new FormData();
      form.append("file", file);
      return authed<OrderDetail>(`/api/orders/${id}/receipts/${receiptId}/attachments`, token, { method: "POST", body: form });
    },
    deleteReceiptProof(token: string, id: number, receiptId: number, attachmentId: number): Promise<OrderDetail> {
      return authed<OrderDetail>(`/api/orders/${id}/receipts/${receiptId}/attachments/${attachmentId}`, token, { method: "DELETE" });
    },
  },
  productTypesCatalog: {
